-- ============================================
-- FIX FINAL: Progreso del Curso - Círculo funcional
-- EJECUTAR EN SUPABASE SQL EDITOR
-- ============================================

-- 1. FUNCIÓN: calculate_module_score (lee de user_progress, BOOL_OR para boolean)
...
BEGIN
  -- Examen: 35% (desde user_progress, NO quiz_attempts)
  ...
  -- Comunidad: 5% (BOOL_OR para boolean, NO MAX)
...
-- 2. FUNCIÓN: get_module_breakdown (lee de user_progress, BOOL_OR)
...
BEGIN
  ...
  -- Comunidad (BOOL_OR para boolean)
  SELECT COALESCE(BOOL_OR(community_comment), FALSE) INTO v_has_comment
  FROM user_progress 
  WHERE user_id = p_user_id AND module_id = p_module_id;

  -- Calcular score global del módulo
...
-- 3. FUNCIÓN: calculate_global_progress
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE ' FIX FINAL: Progreso del Curso';
  RAISE NOTICE '========================================';
  RAISE NOTICE '  calculate_module_score (user_progress + BOOL_OR)';
  RAISE NOTICE '  get_module_breakdown (user_progress + BOOL_OR)';
  RAISE NOTICE '  calculate_global_progress';
  RAISE NOTICE '  trg_update_module_score (trigger)';
  RAISE NOTICE '========================================';
END $$;
