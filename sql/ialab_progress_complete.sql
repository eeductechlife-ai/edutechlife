...
-- ============================================
-- Este script crea todas las tablas, funciones y triggers
-- necesarios para que el sistema de progreso funcione
-- correctamente con persistencia en Supabase.
-- ============================================

-- 1. TABLA: quiz_attempts (si no existe)
...
-- 2. UNIQUE CONSTRAINT en user_progress (para múltiples filas por módulo)
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ Schema de progreso IALab actualizado';
  RAISE NOTICE '========================================';
  RAISE NOTICE '📊 Pesos: Examen 35% + Desafío 30% + Recursos 30% + Comunidad 5% = 100%';
  RAISE NOTICE '📊 Global: 5 módulos × 20% = 100%';
  RAISE NOTICE '';
  RAISE NOTICE 'Funciones creadas:';
  RAISE NOTICE '  - calculate_module_score(user_id, module_id)';
  RAISE NOTICE '  - calculate_global_progress(user_id)';
  RAISE NOTICE '  - get_module_breakdown(user_id, module_id)';
