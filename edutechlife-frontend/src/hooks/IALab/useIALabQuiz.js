...
export const useIALabQuiz = () => {
  ...
  const setSecurityWarningCount = useIALabStore(
    (s) => s.setSecurityWarningCount,
  );
  ...
  const screenshotProtectionActive = useIALabStore(
    (s) => s.screenshotProtectionActive,
  );
  const setScreenshotProtectionActive = useIALabStore(
    (s) => s.setScreenshotProtectionActive,
  );
  ...
  const openEvaluation = useCallback(() => {
    ...
    // El reset va primero: pone `isTimerRunning` en false, así que ejecutarlo
    // después de arrancar el cronómetro lo dejaba apagado (React agrupa los
  ...
  const showSecurityMessageTemporary = useCallback(
    (message) => {
  ...
  const penalizeAttempt = useCallback(() => {
    ...
    useIALabStore
      .getState()
      .storageSet(`exam_attempts_remaining_m${activeMod}`, newRemaining);
    ...
    showSecurityMessageTemporary(
      `¡Penalización! Has perdido ${SECURITY_VIOLATION_PENALTY} intento por infracciones de seguridad`,
    );
  }, [
    activeMod,
    securityViolations,
