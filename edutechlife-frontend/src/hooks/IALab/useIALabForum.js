...
export const useIALabForum = () => {
    ...
    async (limit = 10) => {
      ...
      const promise = (async () => {
        try {
          ...
          if (votesError)
            console.warn("Error loading votes:", votesError.message);
          if (cancelledRef.current) return [];
  ...
  const getForumStats = useCallback(async () => {
    if (!user)
      return { totalPosts: 0, totalComments: 0, totalLikes: 0, activeUsers: 0 };
