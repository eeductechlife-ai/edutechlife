...
export const useForumVotes = () => {
  ...
  const loadVotes = useCallback(
    async (postIds) => {
      ...
      try {
        ...
        postIds.forEach((id) => {
          if (!states[id])
            states[id] = { userVoted: false, voteType: null, isLoading: false };
        });
  ...
  const toggleVote = useCallback(
    async (postId, currentUpvotes) => {
      if (!user) return { success: false, error: "No autenticado" };

      const current = voteStates[postId] || {
        userVoted: false,
        voteType: null,
      };
      ...
      try {
        ...
        setVoteStates((prev) => ({
          ...prev,
          [postId]: {
            userVoted: !isRemovingVote,
            voteType: isRemovingVote ? null : "upvote",
            isLoading: false,
          },
        }));
