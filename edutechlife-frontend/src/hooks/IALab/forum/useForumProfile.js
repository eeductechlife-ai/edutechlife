...
export const useForumProfile = () => {
  ...
  const showHoverProfile = useCallback(
    async (userId) => {
      if (!userId) return;
  ...
  const getLevel = (reputation) => {
    ...
    if (reputation < 50)
      return { level: 2, title: "Aprendiz", color: "#4DA8C4" };
    if (reputation < 100)
      return { level: 3, title: "Contribuidor", color: "#00BCD4" };
    if (reputation < 250)
      return { level: 4, title: "Experto", color: "#10B981" };
  ...
  const getReputationBreakdown = (profile) => {
    ...
    return [
      ...
      {
        label: "Comentarios",
        value: profile.comment_count || 0,
        icon: "fa-reply",
      },
      {
        label: "Likes recibidos",
        value: profile.likes_received || 0,
        icon: "fa-heart",
      },
      {
        label: "Respuestas",
        value: profile.answers_received || 0,
        icon: "fa-check",
      },
    ];
  };
