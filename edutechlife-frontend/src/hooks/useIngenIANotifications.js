...
/**
 * IngenIA NotificationEngine — domain triggers for the student (brief §42).
 *
 ...
 *   - reinforcement   → "IngenIA detectó una oportunidad de refuerzo."
...
export function useIngenIANotifications() {
  const { createNotification, preferences } = useNotification();
  const { dataLoaded, missions, subjectsWithGrades } = useIngenIAKids();
