...
export function useLeadManagement() {
  ...
  const updateLeadInfo = useCallback((updates) => {
    setCurrentLead((prev) => (prev ? { ...prev, ...updates } : prev));
  }, []);

  const addMessageToLead = useCallback(
    (leadId, message) => {
  ...
  const getLeadsByDate = useCallback(
    (date) => {
      return leads.filter((lead) => lead.timestamp.startsWith(date));
    },
    [leads],
  );
