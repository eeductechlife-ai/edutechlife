...
const LeaderboardWidget = memo(({
  ...
  const getRankBgColor = (rank) => {
    if (rank === 1) return 'bg-gradient-to-r from-yellow-100 to-yellow-50 border-l-4 border-yellow-500';
    if (rank === 2) return 'bg-gradient-to-r from-gray-100 to-gray-50 border-l-4 border-gray-400';
    if (rank === 3) return 'bg-gradient-to-r from-orange-100 to-orange-50 border-l-4 border-orange-400';
    ...
    <GlassCard className={`${className}`} animate>
        ...
        <div className="flex gap-2">
          {['weekly', 'monthly', 'all_time'].map(p => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
            >
              {p === 'weekly' ? 'Semanal' : p === 'monthly' ? 'Mensual' : 'Todo'}
            </motion.button>
          ))}
        </div>
      </div>
      ...
      {!loading && displayedLeaderboard.length > 0 ? (
        ...
        <div className="py-8 text-center">
          ...
          <p className="text-gray-500">Sin datos de leaderboard disponibles</p>
        </div>
      ) : null}
    </GlassCard>
  );
