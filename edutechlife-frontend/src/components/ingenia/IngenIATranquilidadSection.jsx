...
IngenIATranquilidadSection.propTypes = {
  t: PropTypes.func.isRequired,
  tranquilidad: PropTypes.array.isRequired,
};
...
export default function IngenIATranquilidadSection({ t, tranquilidad }) {
  const prefersReducedMotion = useReducedMotion();
  return (
    <section
      id="tranquilidad"
      role="region"
      aria-label="Tranquilidad para padres"
      className="relative w-full overflow-hidden bg-gradient-to-b from-petroleum to-petroleum-dark py-12 lg:py-16 scroll-mt-20"
    >
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          {tranquilidad.map((item) => (
            <motion.div
              key={item.title}
              variants={prefersReducedMotion ? undefined : childVariant}
            >
              <TiltCard>
                <div className="group relative bg-white/70 backdrop-blur-xl border border-white/40 shadow-sm rounded-2xl p-5 hover:bg-white/80 transition-all duration-300 h-fit">
                  <div
                    className="w-11 h-11 rounded-2xl bg-gradient-to-br from-primary-light/30 to-mint/20 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300"
        ...
        <motion.div
          ...
          transition={
            prefersReducedMotion
              ? undefined
              : { type: "spring", stiffness: 100, damping: 16, delay: 0.3 }
          }
