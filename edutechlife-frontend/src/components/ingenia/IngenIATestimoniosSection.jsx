...
export default function IngenIATestimoniosSection({ t, testimonials }) {
  ...
  return (
    <section
      ...
      className="relative w-full overflow-hidden bg-gradient-to-b from-white to-petroleum/3 py-12 lg:py-16 scroll-mt-20"
    >
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="relative"
        >
          <div className="absolute left-0 top-0 bottom-0 w-10 md:w-32 z-10 bg-gradient-to-r from-white via-white/80 to-transparent pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-10 md:w-32 z-10 bg-gradient-to-l from-white via-white/80 to-transparent pointer-events-none" />
                  ...
                  (testimonial, idx) => (
                    ...
                    >
                        ...
                        {Array.from({ length: testimonial.rating || 5 }).map(
                          (_, j) => (
                            <Icon
                              key={j}
                              name="fa-star"
                              className="text-corporate text-[11px]"
                            />
                          ),
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-3 pt-4 border-t border-white/40">
                        <div
                          className={`w-10 h-10 rounded-full bg-gradient-to-br ${AVATAR_GRADIENTS[idx % AVATAR_GRADIENTS.length]} flex items-center justify-center text-white font-bold text-sm flex-shrink-0 shadow-md`}
                        >
                          {testimonial.name.charAt(0)}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-petroleum truncate">
                            {testimonial.name}
                          </p>
...
IngenIATestimoniosSection.propTypes = {
