...
const AutomationReadinessTest = ({ onComplete }) => {
  ...
  if (result) {
    return (
      ...
      >
        <div className="bg-white/70 backdrop-blur-xl border border-white/40 shadow-premium rounded-2xl p-8 md:p-10 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold mb-4" style={{ background: `${result.color}15`, color: result.color }}>
            ...
            <h3 className="text-2xl font-black text-petroleum mb-2">
          ...
          <div className="text-left bg-white/70 backdrop-blur-xl border border-white/40 rounded-2xl p-6 mb-6">
            <h4 className="font-bold text-petroleum text-sm mb-3">{t('automation.test.acciones')}:</h4>
          ...
          <div className="flex items-center justify-center gap-2 px-4 py-2 bg-petroleum/5 border border-petroleum/10 rounded-xl mb-6">
            <svg className="w-4 h-4 text-primary-light" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944" /></svg>
            <span className="text-xs font-semibold text-petroleum">{result.standards}</span>
          </div>

          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              onClick={() => onComplete?.(result)}
              className="px-6 py-3 bg-gradient-to-r from-petroleum to-primary-light text-white rounded-full font-bold text-sm hover:shadow-premium transition-all"
            >
              Generar Plan Personalizado
            </button>
            <button
              onClick={handleRestart}
              className="px-6 py-3 border border-white/40 text-slate-600 rounded-full font-semibold text-sm hover:bg-white/80 transition-all"
            >
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="bg-white/70 backdrop-blur-xl border border-white/40 shadow-premium rounded-2xl p-8 md:p-10">
        <div className="flex items-center justify-between mb-6">
          <div>
            <span className="text-xs font-semibold text-primary-light uppercase tracking-wider">
              Categoría: {currentQ.category}
            </span>
            <h3 className="text-lg font-bold text-petroleum mt-1">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <p className="text-xl font-bold text-petroleum mb-6">{currentQ.text}</p>
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={handlePrev}
            disabled={step === 0}
            className="px-5 py-2.5 border border-white/40 text-slate-600 rounded-full text-sm font-semibold hover:bg-white/80 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            className="px-6 py-2.5 bg-gradient-to-r from-petroleum to-primary-light text-white rounded-full text-sm font-bold hover:shadow-premium transition-all disabled:opacity-30 disabled:cursor-not-allowed"
          >
