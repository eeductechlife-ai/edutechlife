...
export default function SmartBoardHeroSection({ t, onBack, inView, countStudents, countImprovement, countHours, handleCta }) {
  const STATS = [
    { value: `+${countStudents.toLocaleString()}`, label: t('smartboard.landing_stat_students'), icon: 'fa-graduation-cap', color: 'text-petroleum' },
    { value: `${countImprovement}%`, label: t('smartboard.landing_stat_improvement'), icon: 'fa-chart-line', color: 'text-primary-light' },
    { value: `+${countHours.toLocaleString()}`, label: t('smartboard.landing_stat_hours'), icon: 'fa-clock', color: 'text-mint' },
  ];

  const PAIN_POINTS = [
    { text: t('smartboard.landing_pain_point1'), icon: 'fa-exclamation-triangle', color: 'bg-petroleum/5 border-petroleum/10 text-petroleum' },
    { text: t('smartboard.landing_pain_point2'), icon: 'fa-gamepad', color: 'bg-primary-light/5 border-primary-light/10 text-primary-light' },
    { text: t('smartboard.landing_pain_point3'), icon: 'fa-eye-slash', color: 'bg-mint/5 border-mint/10 text-mint' },
    { text: t('smartboard.landing_pain_point4'), icon: 'fa-circle-exclamation', color: 'bg-corporate/5 border-corporate/10 text-corporate' },
  ];
  ...
  const vakBadges = [
    { icon: 'fa-eye', label: 'Visual', gradient: 'from-primary-light to-corporate', shadowColor: 'shadow-primary-light/30', style: { top: '-8%', left: '50%', transform: 'translateX(-50%)', zIndex: 20 }, delay: 0.5 },
    { icon: 'fa-ear-listen', label: 'Auditivo', gradient: 'from-corporate to-mint', shadowColor: 'shadow-corporate/30', style: { bottom: '8%', left: '-3%', zIndex: 20 }, delay: 0.7 },
    { icon: 'fa-hand', label: 'Kinestésico', gradient: 'from-mint to-petroleum', shadowColor: 'shadow-mint/30', style: { bottom: '8%', right: '-3%', zIndex: 20 }, delay: 0.9 },
  ];

  const brainDots = [
    { className: 'top-3 left-5 w-2 h-2', delay: 0.5, color: 'bg-white/30' },
    { className: 'bottom-4 right-4 w-1.5 h-1.5', delay: 1, color: 'bg-white/25' },
    { className: 'top-6 right-7 w-1 h-1', delay: 1.5, color: 'bg-white/20' },
    { className: 'bottom-6 left-4 w-1.5 h-1.5', delay: 2, color: 'bg-white/20' },
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-8 py-16 lg:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12 items-center">
          <div className="lg:col-span-3">
            <motion.div {...springUp(0)} className="flex flex-col items-center lg:items-start text-center lg:text-left">
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-petroleum tracking-tight leading-[1.05] mb-4 max-w-4xl">
                {t('smartboard.landing_hero_line1')}<br />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {PAIN_POINTS.map((tag, i) => (
                  <motion.div
                    ...
                    transition={{ type: 'spring', stiffness: 120, damping: 14, delay: 0.35 + i * 0.08 }}
                    className="group relative flex items-center gap-3 px-4 py-3 rounded-xl bg-white/70 backdrop-blur-xl border border-white/40 shadow-sm hover:shadow-premium hover:-translate-y-0.5 transition-all duration-300 cursor-default overflow-hidden"
                  >
                    <div className="absolute left-0 top-0 bottom-0 w-[3px] opacity-60" style={{ backgroundColor: PAIN_COLORS[i].accent }} />
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: PAIN_COLORS[i].bg }}>
                      <Icon name={tag.icon} className="text-sm" style={{ color: PAIN_COLORS[i].accent }} />
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-bold leading-tight mb-0.5" style={{ color: PAIN_COLORS[i].accent }}>{tag.text}</p>
                      <p className="text-[10px] text-slate-400 leading-tight">{PAIN_SOLUTIONS[i]}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
            ...
            >
              <MagneticButton
                onClick={handleCta}
                className="group relative overflow-hidden flex items-center justify-center gap-2.5 w-full sm:w-auto px-8 sm:px-10 py-3.5 rounded-full text-sm sm:text-base font-bold bg-gradient-to-r from-petroleum to-primary-light text-white shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300"
              >
                <span className="absolute inset-0 w-[200%] h-full -translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent group-hover:animate-sweep skew-x-[-20deg]" />
          ...
          <div className="lg:col-span-2 hidden lg:flex flex-col items-center justify-center gap-6">
            <SmartBoardTilt3D intensity={8} className="relative w-[320px] h-[320px] xl:w-[360px] xl:h-[360px] shrink-0">
              <motion.div
              ...
              <div style={{ transform: 'translateZ(0)', transformStyle: 'preserve-3d' }} className="absolute inset-0">
                  ...
                  {[...Array(3)].map((_, i) => (
                    <g key={i} style={{ transformOrigin: '150px 150px' }}>
                      <motion.circle
                        ...
                        style={{ willChange: 'transform', transformOrigin: '150px 150px' }}
                        transition={{
                          rotate: { duration: 25 + i * 12, repeat: Infinity, ease: 'linear' },
                          opacity: { duration: 4 + i, repeat: Infinity, ease: 'easeInOut', delay: i * 1.5 },
                        }}
                      />
                    </g>
                  ))}
              ...
              >
                ...
                >
                  <motion.div style={{ transform: 'translateZ(10px)' }} className="flex flex-col items-center gap-1">
                    <Icon name="fa-brain" className="text-[10rem] xl:text-[14rem] text-white/95" />
                    <span className="text-[9px] font-bold text-white/70 uppercase tracking-[0.2em]">{t('smartboard.landing_hero_brain_label')}</span>
                  </motion.div>
                </motion.div>
              ...
              >
                <motion.div
                  ...
                  style={{ willChange: 'transform', transformStyle: 'preserve-3d' }}
                >
                  <Icon name="fa-star" className="text-xl text-corporate drop-shadow-lg" />
                </motion.div>
              </motion.div>
              ...
              <motion.div
                ...
                initial={{ opacity: 0, y: 10 }}
                animate={inView ? { opacity: 1, y: 0 } : {}}
                transition={{ type: 'spring', stiffness: 100, damping: 14, delay: 1.3 }}
              >
                <motion.div
                  ...
                  style={{ willChange: 'transform', transformStyle: 'preserve-3d' }}
                >
                  <Icon name="fa-rocket-launch" className="text-xl text-mint drop-shadow-lg" />
                </motion.div>
              </motion.div>
                </SmartBoardTilt3D>
            ...
            >
              {STATS.map((stat, i) => (
                <SmartBoardTilt3D key={i} intensity={12}>
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/70 backdrop-blur-xl border border-white/40 shadow-sm min-w-[100px]">
                    <div className="w-8 h-8 rounded-lg bg-white/80 border border-white/40 flex items-center justify-center shrink-0">
                      <Icon name={stat.icon} className={`text-xs ${stat.color}`} />
                    </div>
                    <div>
                      <div className={`text-sm font-black ${stat.color}`}>{stat.value}</div>
                      <div className="text-[9px] text-slate-500 uppercase tracking-widest whitespace-nowrap">{stat.label}</div>
                    </div>
                  </div>
            </SmartBoardTilt3D>
              ))}
