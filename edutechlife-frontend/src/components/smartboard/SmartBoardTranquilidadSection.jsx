...
const TiltCard = ({ children }) => {
  ...
  return (
    <motion.div
      ...
      style={prefersReducedMotion ? {} : { rotateX, rotateY, transformStyle: 'preserve-3d', perspective: 800, willChange: 'transform' }}
    >
      {children}
    </motion.div>
  );
...
SmartBoardTranquilidadSection.propTypes = {
...
export default function SmartBoardTranquilidadSection({ t, tranquilidad }) {
  const prefersReducedMotion = useReducedMotion();
  return (
    <section id="tranquilidad" role="region" aria-label="Tranquilidad para padres" className="relative w-full overflow-hidden bg-gradient-to-b from-petroleum to-petroleum-dark py-12 lg:py-16 scroll-mt-20">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          {tranquilidad.map((item) => (
            <motion.div key={item.title} variants={prefersReducedMotion ? undefined : childVariant}>
              <TiltCard>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={prefersReducedMotion ? undefined : { type: 'spring', stiffness: 100, damping: 16, delay: 0.3 }}
