...
import { fadeInUp, containerVariants, childVariant } from "./IngenIAShared";

export default function IngenIAPlanesSection({
  t,
  pricingPlans,
  paymentMethods,
  guarantee,
  ...
  return (
    <section
      ...
      className="relative w-full overflow-hidden bg-gradient-to-b from-bg-light to-white py-12 lg:py-16 scroll-mt-20"
    >
          ...
          {pricingPlans.map((plan) => (
            ...
            >
              ...
              <div className="text-center mb-6">
                <h3
                  className={`text-xl lg:text-2xl font-bold mb-1 ${plan.popular ? "text-white" : "text-petroleum"}`}
                >
                  {plan.name}
                </h3>
                <div className="flex items-baseline justify-center gap-1">
                  <span
                    className={`text-4xl lg:text-5xl font-black ${plan.popular ? "text-white" : "text-petroleum"}`}
                  >
                    {plan.price}
                  </span>
                  <span
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {plan.features.map((feat) => (
                  <li key={feat} className="flex items-start gap-3">
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${plan.popular ? "bg-white/15" : "bg-success/10"}`}
                    >
                      <Icon
                        name="fa-check"
                        className={`text-[10px] ${plan.popular ? "text-white" : "text-success"}`}
                      />
                    </div>
                    <span
                      className={`text-sm leading-snug ${plan.popular ? "text-white/80" : "text-slate-600"}`}
                    >
                      {feat}
                    </span>
                  </li>
        ...
        >
          <div className="inline-flex flex-wrap items-center justify-center gap-3 px-6 py-4 rounded-2xl bg-white/70 backdrop-blur-xl border border-white/40 shadow-sm">
            {paymentMethods.map((method, i) => (
              ...
              >
                <div
                  className={`w-7 h-7 rounded-lg bg-gradient-to-br from-petroleum/10 to-primary-light/10 flex items-center justify-center`}
                >
        ...
        >
          <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-success/10 border border-success/20 hover:bg-success/15 transition-colors">
            ...
            <div className="text-left">
              <span className="text-xs font-bold text-petroleum block">
                {guarantee.title}
              </span>
...
IngenIAPlanesSection.propTypes = {
