...
const BENEFIT_GRADIENTS = [
  { bg: 'from-petroleum/10 to-primary-light/10', icon: 'from-petroleum to-primary-light' },
  { bg: 'from-primary-light/10 to-corporate/10', icon: 'from-primary-light to-corporate' },
  { bg: 'from-corporate/10 to-mint/10', icon: 'from-corporate to-mint' },
  { bg: 'from-mint/10 to-petroleum/10', icon: 'from-mint to-petroleum' },
  { bg: 'from-petroleum-dark/10 to-petroleum/10', icon: 'from-petroleum-dark to-petroleum' },
  { bg: 'from-primary-light/10 to-mint/10', icon: 'from-primary-light to-mint' },
];

export default function SmartBoardBeneficiosSection({ t, beneficios }) {
  return (
    <section id="beneficios" className="relative w-full overflow-hidden bg-white py-12 lg:py-16 scroll-mt-20">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div {...fadeInUp} className="text-center mb-10">
          ...
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-black text-petroleum leading-tight">
            {t('smartboard.landing_benefits_title_line1')}<br />
            <span className="text-gradient-accent pr-1">{t('smartboard.landing_benefits_title_line2')}</span>
          </h2>
        </motion.div>
        ...
        >
          {beneficios.map((item, i) => (
            <motion.div key={item.title} variants={childVariant}>
              <TiltCard className="group relative bg-white/70 backdrop-blur-xl border border-white/40 shadow-sm hover:shadow-premium-lg transition-shadow duration-300 p-6 h-fit">
                <div className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${BENEFIT_GRADIENTS[i % BENEFIT_GRADIENTS.length].bg} flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300`}>
                  <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${BENEFIT_GRADIENTS[i % BENEFIT_GRADIENTS.length].icon} flex items-center justify-center`}>
