...
import { useIngenIAStats } from "../../hooks/useIngenIAStats";
...
const IngenIAStatsPage = () => {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const stats = useIngenIAStats();
                  ...
                  >
                    <div
                      className={`w-12 h-12 mx-auto mb-3 rounded-2xl flex items-center justify-center ${
                        isLocked ? "bg-slate-100" : "bg-primary-light/10"
                      }`}
                    >
                      <IconComp
                        className={`w-5 h-5 ${
...
export default IngenIAStatsPage;
