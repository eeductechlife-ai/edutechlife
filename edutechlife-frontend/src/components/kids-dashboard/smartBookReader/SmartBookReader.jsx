...
const SmartBookReader = () => {
  const { darkMode, mode, text, setText, step, book, view, setView, error, process, history, reset } = useBookReader();

  return (
    <div className="space-y-5">
      <div>
        <h3 className={`text-lg font-bold ${darkMode ? "text-white" : "text-[#004B63]"}`}>
          📖 SmartBook
        </h3>
      ...
      <AnimatePresence mode="wait">
        {mode === "input" && (
          ...
          >
            <div>
              <label className={`text-sm font-semibold mb-2 block ${darkMode ? "text-white" : "text-[#004B63]"}`}>
                Pega tu texto aquí
              </label>
              <textarea
                value={text}
            ...
            <div className="flex items-center gap-3">
              ...
              <span className={`text-xs font-semibold ${darkMode ? "text-[#64748B]" : "text-[#94A3B8]"}`}>
                o
              </span>
        ...
        {view && (
          ...
          >
            ...
            >
              ...
              <button onClick={() => setView(null)} className="mt-2 w-full py-2 text-sm text-white/80 hover:text-white text-center">
                Cerrar
              </button>
            </motion.div>
          </motion.div>
      ...
      <div>
        <h4 className={`text-sm font-bold mb-3 ${darkMode ? "text-white" : "text-[#004B63]"}`}>
          📚 Historial ({history.length})
        </h4>
        {history.length === 0 ? (
          <div className="text-center py-8">
            <span className="text-5xl mb-3 block">📚</span>
            <p className={`text-sm font-semibold ${darkMode ? "text-white" : "text-[#004B63]"}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-60 overflow-y-auto pr-1">
            {history.map((b, i) => (
              <HistoryItem key={b.id} book={b} i={i} dark={darkMode} onSelect={setView} />
            ))}
          </div>
        )}
      </div>
