...
const CONFIDENCE_LEVELS = [
  {
    emoji: '😖', label: 'No lo sé', points: 0, color: '#EF4444',
  ...
  {
    emoji: '😕', label: 'Casi', points: 1, color: '#F59E0B',
  ...
  {
    emoji: '😊', label: 'Lo sabía', points: 2, color: '#4DA8C4',
  ...
  {
    emoji: '🤩', label: 'Lo domino', points: 3, color: '#10B981',
...
const glassBorder = (color) => `0 0 0 1px ${color}20 inset, 0 0 0 1px rgba(255,255,255,0.6) inset`;
const glassShadow = (color) => `0 8px 32px ${color}15, 0 1px 4px ${color}08`;

const QuizCard = memo(({ card, flipped, onFlip, onResult, idx, total, categoryColor, topicIcons = ['fa-brain', 'fa-lightbulb', 'fa-graduation-cap', 'fa-star', 'fa-sparkles'] }) => {
  ...
  const iconBack = useMemo(() => pickIcon(topicIcons, card.front.length + card.back.length + 3), [topicIcons, card.front, card.back]);
    ...
    <div className="flex flex-col items-center gap-6 w-full">
      ...
      <div className="flex items-center w-full max-w-xl px-1">
        <div className="flex items-center gap-2.5 text-sm">
          <span className="font-black text-lg tracking-tight" style={{ color: categoryColor }}>{idx + 1}</span>
        ...
        <div className="flex-1 mx-4 h-1.5 bg-[#E2E8F0]/60 rounded-full overflow-hidden">
          <motion.div
            ...
            style={{ backgroundColor: categoryColor, boxShadow: `0 0 6px ${categoryColor}55` }}
            initial={{ width: 0 }}
        ...
        <div className="flex items-center gap-1.5">
          {supported && (
            <motion.button
              ...
              title={speaking ? 'Detener' : 'Dani lee esta tarjeta'}
            >
              {speaking ? (
                <motion.div
        ...
        >
          ...
          >
            ...
            <div className="absolute -top-16 -right-16 w-48 h-48 rounded-full pointer-events-none"
              style={{ background: `radial-gradient(circle, ${categoryColor}18, transparent 70%)` }} />
            <div className="absolute -bottom-12 -left-12 w-40 h-40 rounded-full pointer-events-none"
              style={{ background: `radial-gradient(circle, ${categoryColor}10, transparent 70%)` }} />
            ...
            <div className="absolute inset-0 pointer-events-none"
              style={{
                background: 'linear-gradient(135deg, rgba(255,255,255,0.35) 0%, transparent 50%, rgba(255,255,255,0.08) 100%)',
              }} />

            {/* Top accent */}
            <motion.div className="absolute top-0 left-0 right-0 h-[3px] rounded-full"
              style={{ background: `linear-gradient(90deg, ${categoryColor}, ${categoryColor}66, transparent)` }}
              initial={{ scaleX: 0, originX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.6, ease: 'easeOut' }} />

            {/* Icon cluster */}
            <div className="relative z-10 flex items-center justify-center mb-3" style={{ width: 80, height: 80 }}>
              {/* Primary icon */}
              <motion.div
                key={iconFront}
                ...
                style={{ filter: `drop-shadow(0 4px 12px ${categoryColor}30)` }}
              >
                <Icon name={iconFront} className="w-14 h-14 sm:w-16 sm:h-16" style={{ color: categoryColor }} />
              </motion.div>
              ...
              {decorIcons.map((iconName, i) => {
                ...
                return (
                  <motion.div
                    ...
                    style={{ top: `calc(50% + ${posY}px)`, left: `calc(50% + ${posX}px)` }}
                    initial={{ scale: 0, opacity: 0 }}
                    animate={{ scale: 1, opacity: 0.3 }}
                    transition={{ delay: 0.15 + i * 0.1, type: 'spring', stiffness: 200, damping: 16 }}
                  >
                    <Icon name={iconName} className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: categoryColor }} />
                  </motion.div>
                );
              })}
            </div>
            ...
            {/* Badge */}
            <span className="relative z-10 px-3.5 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.18em] mb-3"
              style={{
            ...
            {/* Term */}
            <p className="relative z-10 text-xl sm:text-2xl font-black text-center leading-snug max-w-xs px-4" style={{ color: '#004B63' }}>
              {card.front}
            </p>
          ...
          >
            {/* Ambient glow */}
            <div className="absolute -top-12 -left-12 w-36 h-36 rounded-full pointer-events-none"
              style={{ background: `radial-gradient(circle, ${categoryColor}12, transparent 70%)` }} />
            ...
            >
              <Icon name={iconBack} className="w-10 h-10 sm:w-12 sm:h-12" style={{ color: categoryColor }} />
            </motion.div>
            ...
            {/* Definition */}
            <p className="relative z-10 text-base sm:text-lg font-bold text-center leading-relaxed max-w-sm px-5" style={{ color: '#004B63' }}>
              {card.back}
            </p>
          </div>
        </motion.div>
          ...
          >
            {CONFIDENCE_LEVELS.map((level) => (
              ...
              >
                ...
                <span className="text-[11px] font-black whitespace-nowrap tracking-tight" style={{ color: level.color }}>
                  {level.label}
                </span>
              </motion.button>
            ))}
