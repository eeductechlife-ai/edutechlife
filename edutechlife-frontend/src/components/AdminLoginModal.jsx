...
const AdminLoginModal = ({ isOpen, onClose, onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // In production, authentication is handled by Clerk provider
    // This demo modal requires actual Clerk JWT verification on backend
    ...
    // Placeholder: Replace with actual backend auth verification
    try {
      // TODO: Integrate with backend JWT verification
      setError(t("adminLogin.invalid_credentials"));
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t("adminLogin.restricted_access")}
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        <div
          ...
          style={{
            background: "radial-gradient(circle, #004B63, transparent)",
            filter: "blur(60px)",
          }}
        ></div>
        <div
          ...
          style={{
            background: "radial-gradient(circle, #4DA8C4, transparent)",
            filter: "blur(60px)",
          }}
        ></div>
      </div>
      ...
      <div className="relative w-full max-w-md">
        <div
          ...
          style={{
            background: "linear-gradient(135deg, #004B63, #4DA8C4, #66CCCC)",
            filter: "blur(20px)",
          }}
        ></div>

        <div
          className="relative rounded-3xl p-8 border border-[#004B63]/50"
          style={{
            background:
              "linear-gradient(135deg, rgba(11, 15, 25, 0.98) 0%, rgba(0, 75, 99, 0.4) 100%)",
          }}
        >
          <button
            onClick={onClose}
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-[#B2D8E5]">
              {t("adminLogin.restricted_access")}
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-2 mb-6 px-4 py-2 rounded-xl bg-[#66CCCC]/10 border border-[#66CCCC]/20">
            ...
            <span className="text-xs text-[#66CCCC] font-semibold">
              {t("adminLogin.ssl_connection")}
            </span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label
                ...
                className="block text-sm font-semibold text-[#B2D8E5] mb-2"
              >
