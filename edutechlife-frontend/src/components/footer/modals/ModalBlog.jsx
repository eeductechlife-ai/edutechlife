...
export default function ModalBlog({ onClose, content }) {
  ...
  if (selectedArticle) {
    ...
    return (
      <div ref={focusTrapRef} role="dialog" aria-modal="true" aria-label={t('footer.blog')} className="fixed inset-0 z-[100] flex items-center justify-center p-2 md:p-4" onClick={() => setSelectedArticle(null)}>
        ...
        <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white" onClick={(e) => e.stopPropagation()}>
          <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b" style={{ backgroundColor: '#004B63', borderColor: '#003d52' }}>
            ...
            <button onClick={() => setSelectedArticle(null)} aria-label="Cerrar" className="w-9 h-9 rounded-full flex items-center justify-center transition-colors hover:bg-white/20" style={footerWhiteText}>
              <Icon name="fa-xmark" className="text-lg" />
            </button>
          </div>

          <div className="relative h-56 md:h-72 overflow-hidden">
            <img src={article.imagen} alt={article.titulo} className="w-full h-full object-cover" />
          ...
          <div className="px-6 md:px-10 py-6 space-y-8">
            <p className="text-lg leading-relaxed" style={{ color: '#1F2937' }}>{article.introduccion}</p>
            {article.secciones.map((seccion, idx) => (
              <div key={idx} className="space-y-4">
                ...
                {seccion.grafica === 'linea' && renderLineChart(seccion, footerPrimaryText, footerMutedText, footerLightBg)}
                {seccion.grafica === 'barras' && renderBarChart(seccion, footerPrimaryText, footerDarkText, footerLightBg)}
                {seccion.grafica === 'dona' && renderDonutChart(seccion, footerDarkText, footerLightBg)}
              </div>
            ))}
          ...
          <div className="px-6 py-4 border-t flex items-center justify-between" style={{ borderColor: '#E5E7EB', backgroundColor: '#F9FAFB' }}>
            ...
            <button onClick={() => setSelectedArticle(null)} className="px-4 py-2 rounded-lg text-sm font-medium transition-colors" style={footerPrimaryButton}
  ...
  return (
    <div ref={focusTrapRef} role="dialog" aria-modal="true" aria-label={t('footer.blog')} className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl p-6 md:p-8 bg-white" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          {blogArticles.map((articulo, index) => (
            <div key={index} className="p-4 rounded-xl border-2 transition-all hover:shadow-lg cursor-pointer" style={{ borderColor: '#E8F4F8', backgroundColor: '#FAFDFF' }} onClick={() => setSelectedArticle(articulo.id)}>
              <div className="flex items-center gap-3 mb-2">
                <span className="px-2 py-1 rounded text-xs font-medium" style={{ backgroundColor: '#E8F4F8', color: '#004B63' }}>{articulo.categoria}</span>
              ...
              <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                <span>{articulo.autor}</span><span>•</span><span>{articulo.tiempoLectura}</span>
              </div>
            </div>
          ))}
          <div className="mt-6 text-center">
            <button className="px-6 py-3 rounded-xl font-semibold transition-all" style={footerPrimaryButton}
              onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#003d52'}
              onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#004B63'}
            >
  ...
  return (
    <div className="p-4 rounded-xl" style={{ backgroundColor: '#F3F9FB' }}>
      ...
      <div className="space-y-3">
        {seccion.datos.map((d, i) => {
          const max = Math.max(...seccion.datos.map(x => x.valor || x.mejora || x.despues || 100));
