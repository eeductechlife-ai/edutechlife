...
const SupabaseLoginForm = ({ returnTo = "/ialab" }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.message || t("login.error.invalid_credentials") || "Invalid credentials");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <form onSubmit={handleLogin} className="space-y-3">
        ...
        >
          {loading ? t("login.button.signing_in") || "Iniciando sesión..." : t("login.button.signin") || "Iniciar Sesión"}
        </button>
      </form>
