...
const SEO = ({
  ...
  const image =
    ogImage ||
    (isEn ? "/og-image-en.svg" : isPt ? "/og-image-pt.svg" : "/og-image.svg");
  const type = ogType || "website";
  const ogLocale = isEn ? "en_US" : isPt ? "pt_BR" : "es_CO";
  const currentLang = lang || (isEn ? "en" : isPt ? "pt" : "es");
  const pathNoLocale = location.pathname.replace(/^\/(en|pt)/, "");
    ...
    <>
      <Helmet>
        ...
        <link
          rel="alternate"
          hrefLang="en"
          href={`${SITE_URL}/en${pathNoLocale}`}
        />
        <link
          rel="alternate"
          hrefLang="pt"
          href={`${SITE_URL}/pt${pathNoLocale}`}
        />
        <link
          rel="alternate"
          hrefLang="x-default"
