...
export function TabPills({ TABS, viewSection, setViewSection }) {
  const handleKeyDown = useCallback((e, tabIndex) => {
    ...
    document.querySelector(`[data-tab-id="${tabs[nextIdx].id || 'all'}"]`)?.focus();
    ...
    <>
      {TABS.map((tab) => (
        <button
          ...
          className={`min-h-[44px] flex items-center gap-1 px-2.5 py-1.5 text-[11px] md:px-3.5 md:py-2 md:text-xs font-semibold transition-all duration-300 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-petroleum/40 border rounded-lg md:rounded-xl ${
            viewSection === tab.id
