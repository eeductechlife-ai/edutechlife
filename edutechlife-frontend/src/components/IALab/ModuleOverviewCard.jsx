        ...
        <div className="relative z-10 bg-white/80 backdrop-blur-md rounded-[2rem] border border-cyan-100 shadow-xl p-6 md:p-8 overflow-hidden bg-gradient-to-br from-white/90 to-cyan-50/50 mb-8">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-bold text-[#004B63] mb-3">
                {moduleData.title}
              </h3>
              ...
              {/* Bullet Points - 6 temas horizontales con wrap (AHORA CLICKEABLES) */}
              <div className="flex flex-wrap gap-3 mt-4">
                {moduleData.topics.map((tema, index) => (
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 bg-white/70 border border-slate-100 rounded-xl hover:bg-white hover:shadow-sm transition-all duration-200 cursor-pointer hover:scale-[1.01] active:scale-[0.99] group"
                    aria-label={`Ver recursos del tema: ${tema}`}
                  >
                    <div className="w-2 h-2 rounded-full bg-cyan-500 flex-shrink-0 group-hover:scale-125 transition-transform duration-200"></div>
                    <span className="text-sm text-slate-700 whitespace-nowrap group-hover:text-[#004B63] group-hover:font-medium transition-colors duration-200">
                      {tema}
                    </span>
                    {/* Indicador de interactividad */}
                    <Icon 
                      name="fa-arrow-up-right-from-square" 
                      className="w-3 h-3 text-slate-400 opacity-0 group-hover:opacity-100 transition-all duration-200 ml-1" 
                    />
                  </button>
                ))}
              </div>
