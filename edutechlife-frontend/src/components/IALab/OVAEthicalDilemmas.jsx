...
export default function OVAEthicalDilemmas({ onComplete }) {
  ...
  const handleAnswer = (idx) => {
    ...
    if (idx === dilemmas[currentDilemma].correct) {
      setAnswers((prev) => ({
        ...prev,
        [currentDilemma]: { ...prev[currentDilemma], correct: true },
      }));
    } else {
      setAnswers((prev) => ({
        ...prev,
        [currentDilemma]: { ...prev[currentDilemma], correct: false },
      }));
    }
  };
      ...
      <aside className="w-full md:w-64 bg-white/90 dark:bg-slate-800/90 flex flex-col shadow-xl z-10 md:min-h-full border-r border-cyan-100 dark:border-gray-700">
        <div className="p-6 text-center border-b border-cyan-50 dark:border-gray-700">
          ...
          <p className="text-[10px] uppercase mt-2 text-slate-600 dark:text-slate-300 font-bold tracking-[0.2em]">
            {t(
              "ova.ethical_dilemmas.sidebar_subtitle",
              "Laboratorio de Ética en IA",
            )}
          </p>
        </div>
          ...
          {navItems.map((item) => (
            ...
            >
              ...
              <span className="text-sm md:text-base">
                {t(`ova.ethical_dilemmas.nav_${item.id}`)}
              </span>
            </button>
          ))}
        </nav>
            ...
            <div className="animate-[fadeIn_0.6s_ease-out_forwards]">
              ...
              <div className="w-full h-64 md:h-80 rounded-2xl mb-8 overflow-hidden shadow-xl border border-gray-100 dark:border-gray-700">
                <img
                  ...
                  alt="Dilemas éticos en inteligencia artificial"
                  loading="lazy"
              ...
              <p className="text-lg text-slate-700 dark:text-slate-200 leading-relaxed">
                {t(
                  "ova.ethical_dilemmas.intro_text",
              ...
              <div className="mt-6 p-6 bg-cyan-50 dark:bg-cyan-900/20 border-l-8 border-corporate rounded-r-2xl italic text-petroleum dark:text-slate-100 font-medium">
                ...
                {t(
                  ...
                  "La tecnología no es buena ni mala, pero el uso que hacemos de ella sí tiene consecuencias éticas.",
                )}
                &rdquo;
              </div>
          ...
          {activeSection === "dilemmas" && (
            <div className="animate-[fadeIn_0.6s_ease-out_forwards] flex flex-col h-full">
              ...
              <div className="flex-1 bg-slate-50 dark:bg-slate-700/30 rounded-2xl p-6 md:p-8 border border-slate-100 dark:border-slate-700">
                <div className="mb-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs uppercase tracking-widest text-amber-600 font-bold">
                      {t("ova.ethical_dilemmas.scenario_label", "ESCENARIO")}{" "}
                      {currentDilemma + 1}
                    </span>
                  </div>
                ...
                {showFeedback && (
                  <div
                    className={`mt-6 p-5 rounded-2xl border-l-8 ${selectedAnswer === dilemmas[currentDilemma].correct ? "bg-emerald-50 dark:bg-emerald-900/20 border-emerald-400" : "bg-rose-50 dark:bg-rose-900/20 border-rose-400"}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p
                          className={`font-bold mb-1 ${selectedAnswer === dilemmas[currentDilemma].correct ? "text-emerald-800 dark:text-emerald-200" : "text-rose-800 dark:text-rose-200"}`}
                        >
                          {selectedAnswer === dilemmas[currentDilemma].correct
                            ? t(
                                "ova.ethical_dilemmas.correct_label",
                                "¡Correcto!",
                              )
                            : t(
                                "ova.ethical_dilemmas.incorrect_label",
                                "Incorrecto",
                              )}
                        </p>
                        <p
                          className={`text-sm ${selectedAnswer === dilemmas[currentDilemma].correct ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}`}
                        >
                ...
                {showFeedback && currentDilemma === totalDilemmas - 1 && (
                  ...
                  >
                    {t(
                      "ova.ethical_dilemmas.see_results_btn",
                      "Ver Resultados",
                    )}
                  </button>
                )}
          ...
          {activeSection === "principles" && (
            <div className="animate-[fadeIn_0.6s_ease-out_forwards]">
                ...
                <div className="mb-8 p-6 bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-900/10 rounded-2xl border border-emerald-200 dark:border-emerald-800">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-emerald-800 dark:text-emerald-200">
                        {t(
                          "ova.ethical_dilemmas.results_title",
                          "Tus Resultados",
                        )}
                      </h3>
                      <p className="text-emerald-700 dark:text-emerald-300">
                        {correctCount} / {totalDilemmas}{" "}
                        {t("ova.ethical_dilemmas.correct_count", "correctas")}
                        {correctCount === totalDilemmas
                          ? " " +
                            t(
                              "ova.ethical_dilemmas.perfect_score",
                              "¡Puntuación perfecta!",
                            )
                          : ""}
                      </p>
                  ...
                  <div className="w-full bg-white/50 dark:bg-slate-700/50 rounded-full h-3 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(correctCount / totalDilemmas) * 100}%`,
                      }}
                    />
                  </div>
                </div>
              ...
              <p className="text-base text-slate-600 dark:text-slate-300 mb-8">
                {t(
                  "ova.ethical_dilemmas.principles_text",
                  "Estos son los pilares fundamentales para guiar tus decisiones éticas al usar inteligencia artificial.",
                )}
              </p>
