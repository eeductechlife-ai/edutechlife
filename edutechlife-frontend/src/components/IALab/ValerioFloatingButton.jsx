...
const ValerioFloatingButton = ({ onClick, t, hasStartedCourse = false }) => {
  ...
  const [showNudge, setShowNudge] = useState(() => {
    try {
      return !localStorage.getItem("ialab_valerio_seen");
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      setShowNudge(false);
      try {
        localStorage.setItem("ialab_valerio_seen", "1");
      } catch {}
    }, 15000);
          ...
          >
            <AnimatePresence>
              {showNudge && !showTooltip && (
                <motion.div
                  ...
                  className="absolute right-full mr-3 bottom-full mb-2 sm:bottom-auto sm:top-1/2 sm:-translate-y-1/2 sm:mb-0 pointer-events-none"
                >
                  <div className="bg-white dark:bg-slate-900 shadow-2xl rounded-2xl px-4 py-3 border border-slate-100 dark:border-slate-700 max-w-[200px] sm:max-w-none sm:whitespace-nowrap">
                    <div className="flex items-start sm:items-center gap-2.5">
                      <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-cyan-400 to-[var(--theme-primary)] flex items-center justify-center flex-shrink-0 mt-0.5 sm:mt-0">
                        <Icon
                      ...
                      <div>
                        <p className="text-sm font-bold text-[var(--theme-emphasis)] dark:text-[#4DA8C4]">
                          {hasStartedCourse
                            ? t("ialab.valerio_nudge_return") ||
                              "¡Bienvenido de nuevo!"
                            : t("ialab.valerio_nudge")}
                        </p>
                        <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 leading-snug">
                          {hasStartedCourse
                            ? t("ialab.valerio_nudge_return_sub") ||
                              "Toca para continuar donde lo dejaste"
                            : t("ialab.valerio_nudge_sub")}
                        </p>
                      </div>
                    </div>
                  </div>
                  {/* Flecha apunta al botón — en mobile apunta abajo, en desktop apunta a la derecha */}
                  <div className="hidden sm:block absolute right-[-5px] top-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-white dark:bg-slate-900 border-r border-b border-slate-100 dark:border-slate-700 rotate-45" />
                  <div className="sm:hidden absolute right-4 bottom-[-5px] w-2.5 h-2.5 bg-white dark:bg-slate-900 border-r border-b border-slate-100 dark:border-slate-700 rotate-45" />
                </motion.div>
              )}
            </AnimatePresence>
            <AnimatePresence>
            ...
            <motion.span
              ...
              style={{
                ...
                height: 100,
                background:
                  "radial-gradient(circle, rgba(0,188,212,0.15) 0%, rgba(0,75,99,0.08) 40%, transparent 70%)",
              }}
