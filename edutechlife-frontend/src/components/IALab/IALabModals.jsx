...
const ModalsSection = ({
  handleGlobalAction, activeMod, completedExams
}) => {
  ...
  const modalItems = [
    { key: 'showExamModal', isOpen: showExamModal, children: (
      <SectionErrorBoundary>
        <Suspense fallback={<LoadingFallback />}>
          <IALabEvaluationModal isOpen={showExamModal} onClose={() => handleGlobalAction('CLOSE_EVALUATION')} moduleId={activeMod} />
        </Suspense>
      </SectionErrorBoundary>
    )},
    { key: 'showQuizModal', isOpen: showQuizModal, children: (
      <SectionErrorBoundary>
        <Suspense fallback={<LoadingFallback />}>
          <IALabQuizModal isOpen={showQuizModal} onClose={() => handleGlobalAction('CLOSE_QUIZ')} />
        </Suspense>
      </SectionErrorBoundary>
    )},
    { key: 'showPremiumEvaluationModal', isOpen: showPremiumEvaluationModal, children: (
      <SectionErrorBoundary showDetails>
        <Suspense fallback={<LoadingFallback />}>
          <IALabEvaluationModalPremium isOpen={showPremiumEvaluationModal} onClose={() => setShowPremiumEvaluationModal(false)} />
        </Suspense>
      </SectionErrorBoundary>
    )},
    { key: 'showCertificateModal', isOpen: showCertificateModal, children: (
      <SectionErrorBoundary>
        <Suspense fallback={<LoadingFallback />}>
          <CertificatesModal isOpen={showCertificateModal} onClose={() => setShowCertificateModal(false)} initialTab="certificate" />
        </Suspense>
      </SectionErrorBoundary>
    )},
    { key: 'showBadgeGallery', isOpen: showBadgeGallery, children: (
      <SectionErrorBoundary>
        <Suspense fallback={<LoadingFallback />}>
          <BadgeGalleryModal isOpen={showBadgeGallery} onClose={() => setShowBadgeGallery(false)} />
        </Suspense>
      </SectionErrorBoundary>
    )},
    { key: 'showLeaderboard', isOpen: showLeaderboard, children: (
      <SectionErrorBoundary>
        <Suspense fallback={<LoadingFallback />}>
    ...
    { key: 'showExamResult', isOpen: showExamResult, children: (
      <SectionErrorBoundary>
        <Suspense fallback={<LoadingFallback />}>
          <ExamResultViewer moduleId={activeMod} score={completedExams[activeMod]}
            onClose={() => setShowExamResult(false)}
            onRetry={() => { setShowExamResult(false); handleGlobalAction('OPEN_QUIZ'); }} />
        </Suspense>
      </SectionErrorBoundary>
    )},
    { key: 'showHistoryModal', isOpen: showHistoryModal, children: (
      <AnimatePresence>
        {showHistoryModal && (
          <SectionErrorBoundary>
            <ActivityHistory isOpen={showHistoryModal} onClose={() => useIALabStore.getState().setShowHistoryModal(false)} />
          </SectionErrorBoundary>
        )}
      </AnimatePresence>
    )},
    { key: 'showHelpModal', isOpen: showHelpModal, children: (
      <AnimatePresence>
        {showHelpModal && (
          <SectionErrorBoundary>
            <SettingsSupportModal isOpen={showHelpModal} onClose={() => useIALabStore.getState().setShowHelpModal(false)} />
          </SectionErrorBoundary>
        )}
      </AnimatePresence>
    )},
    { key: 'showChallengeResult', isOpen: showChallengeResult, children: (
      <SectionErrorBoundary>
        <Suspense fallback={<LoadingFallback />}>
          <ChallengeResultViewer moduleId={activeMod}
