...
const EdutechLogo = () => (
  <div className="flex items-center gap-2 select-none">
    ...
    <div className="text-xl tracking-tighter flex items-center lowercase font-bold">
      <span className="text-petroleum-dark">edu</span><span className="text-corporate">techlife</span>
    </div>
  </div>
);
...
export default function OVARiskSimulator({ onComplete }) {
  ...
  const renderModal = () => {
    ...
    return (
      <div ref={focusTrapRef} role="dialog" aria-modal="true" aria-label={openModal.title} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setOpenModal(null)}>
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            <h3 className="text-lg font-bold text-petroleum-dark dark:text-slate-100">{openModal.title}</h3>
            <button onClick={() => setOpenModal(null)} className={`text-gray-400 hover:text-red-500 text-2xl ${FOCUS_RING} rounded-lg`}>&times;</button>
          </div>
          <p className="text-gray-700 dark:text-slate-300 leading-relaxed mb-6">{openModal.desc}</p>
          <button onClick={() => setOpenModal(null)} className={`w-full bg-gradient-to-r from-corporate to-petroleum-dark text-white font-bold py-3 px-4 rounded-xl ${FOCUS_RING} transition hover:shadow-lg`}>{t('ova.risksim.modal_button')}</button>
        </div>
      </div>
    );
  };
  ...
  const renderStarGame = () => {
    ...
    return (
      <div className="text-center py-4">
        ...
        {showFeedback && (
          <button onClick={resetGame} className={`mt-6 px-6 py-3 bg-gradient-to-r from-corporate to-petroleum-dark text-white rounded-xl font-bold shadow-md hover:shadow-lg ${FOCUS_RING}`}>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        {activeTab === 'content' && (
          <div className="space-y-10 animate-[fadeIn_0.6s_ease-out_forwards]">
            <section className={`${GLASS_CARD} rounded-2xl p-8 text-center border-t-4 border-corporate`}>
            ...
            <section>
              <h2 className="text-2xl font-bold text-petroleum-dark dark:text-slate-100 mb-6 flex items-center gap-2"><Brain size={24} className="text-corporate" /> {t('ova.risksim.section_bias')}</h2>
              {renderAccordion()}
            </section>

            <section>
              <h2 className="text-2xl font-bold text-petroleum-dark dark:text-slate-100 mb-6 flex items-center gap-2">{t('ova.risksim.section_mitigations')}</h2>
            ...
            <section className={`${GLASS_CARD} p-8 rounded-2xl border-l-8 border-corporate`}>
              <h2 className="text-2xl font-bold text-petroleum-dark dark:text-slate-100 mb-4">{t('ova.risksim.decalogue_title')}</h2>
