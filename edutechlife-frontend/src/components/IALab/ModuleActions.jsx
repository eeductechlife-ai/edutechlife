...
const ActionCard = memo(({ icon, label, weightKey, onClick, completed, score, remainingAttempts, color = 'theme-bg-emphasis', t }) => {
  ...
  return (
    <motion.button
      ...
      className={`relative w-full flex items-center gap-4 p-4 rounded-xl border transition-all duration-200 text-left cursor-pointer group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--theme-primary)]/40 ${
        isApproved
          ...
          : 'bg-[var(--theme-surface)] theme-border hover:shadow-md hover:theme-border-emphasis-20'
      }`}
    >
      {isApproved && (
      ...
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm transition-all duration-200 ${
        isApproved
          ...
          : 'theme-bg-emphasis group-hover:shadow-md'
      }`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className={`font-bold text-sm truncate ${
            isApproved ? 'text-emerald-700 dark:text-emerald-400' : isFailed ? 'text-red-700 dark:text-red-400' : 'text-slate-800 dark:text-slate-100 group-hover:theme-text-emphasis'
          }`}>
            {label}
          </span>
        </div>
...
const ModuleActions = ({ onAction, activeMod, challengeScores, completedExams, moduleProgress, isForumOpen, onToggleForum }) => {
  ...
  return (
    <div className="relative rounded-2xl border theme-border shadow-sm p-5 md:p-8" style={{ background: 'var(--theme-surface)' }}>
      <div className="absolute -top-6 -right-6 w-32 h-32 bg-gradient-to-br from-transparent to-transparent rounded-full blur-2xl pointer-events-none"></div>
      <div className="absolute -bottom-6 -left-6 w-32 h-32 bg-gradient-to-tr from-transparent to-transparent rounded-full blur-2xl pointer-events-none"></div>
      <div className="absolute top-0 left-0 right-0 h-1.5 theme-bg-emphasis rounded-t-2xl" />

      <div className="flex items-center gap-3 mb-5">
        <div className="w-10 h-10 rounded-xl theme-bg-emphasis flex items-center justify-center shadow-md flex-shrink-0">
          <Icon name="fa-bolt" className="text-white text-base" />
        </div>
        <div>
          <h4 className="text-sm font-bold theme-text-emphasis uppercase tracking-wider font-montserrat">{t('ialab.module_actions.title')}</h4>
