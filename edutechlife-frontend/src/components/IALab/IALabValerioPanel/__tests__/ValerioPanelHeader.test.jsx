...
vi.mock("../../../ValerioAvatar", () => ({
  default: ({ state }) => (
    <div data-testid="valerio-avatar" data-state={state} />
  ),
}));
...
describe("ValerioPanelHeader", () => {
  test("renders module title", () => {
    ...
    expect(
      screen.getByText(/ialab\.valerio\.module_label/),
    ).toBeInTheDocument();
  });
  ...
  test("shows thinking state indicator", () => {
    ...
    expect(
      screen.getByText(/ialab\.valerio\.status_thinking/),
    ).toBeInTheDocument();
  });
  ...
  test("renders close button", () => {
    render(<ValerioPanelHeader {...defaultProps} />);
    expect(
      screen.getByRole("button", { name: /ialab\.valerio\.close_aria/ }),
    ).toBeInTheDocument();
  });
