...
import { Briefcase, CheckCircle, XCircle, Award, Lightbulb, ArrowRight, ArrowLeft, Target, Brain, ChevronRight, Star, Building2 } from 'lucide-react';
...
const OVAPracticalCases = ({ onComplete }) => {
  ...
  const handleAnswer = (idx) => {
    ...
    setAnswers(prev => ({
      ...prev,
      [currentChallenge]: { correct: idx === challenges[currentChallenge].correct }
    }));
  };
        ...
        <div className="p-4 space-y-2">
          ...
          {challenges.map((ch, i) => (
            ...
            >
              ...
              {i === currentChallenge && <ChevronRight size={14} className="ml-auto" />}
            </button>
          ))}
        </div>
            ...
            <div className="flex gap-1">
              {challenges.map((_, i) => (
                <div key={i} className={`w-2.5 h-2.5 rounded-full transition-all ${
                  answers[i]?.correct ? 'bg-emerald-400' :
                  answers[i] ? 'bg-rose-400' :
                  i === currentChallenge ? 'bg-corporate scale-125' :
          ...
          <div className="flex-1 bg-slate-50 dark:bg-slate-700/30 rounded-2xl p-6 md:p-8 border border-slate-100 dark:border-slate-700">
            ...
            <div className="space-y-3">
              ...
              {c.options.map((opt, idx) => {
                ...
                const isOptCorrect = idx === challenges[currentChallenge].correct;
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5
                        ${showResult && isOptCorrect ? 'bg-emerald-500 text-white' :
                        ...
                        {showResult && isOptCorrect ? <CheckCircle size={14} /> :
            ...
            {showFeedback && (
              <div className={`mt-6 p-5 rounded-2xl border-l-8 ${
                isCorrect
