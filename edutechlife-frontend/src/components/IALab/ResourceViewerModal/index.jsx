      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-[201] flex items-center justify-center p-2 sm:p-4 pointer-events-none">
            <div className="w-full max-w-6xl h-[90dvh] max-h-[900px] mx-2 sm:mx-4 p-[1.5px] rounded-[2rem] bg-gradient-to-b from-petroleum/20 via-petroleum/10 to-corporate/5 relative overflow-hidden shadow-[0_20px_25px_-5px_rgba(0,75,99,0.18),0_8px_10px_-6px_rgba(0,75,99,0.12)]">
              ...
              >
                ...
                <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0 w-full sm:w-auto">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-petroleum to-corporate shadow-sm flex items-center justify-center flex-shrink-0"><Icon name={RESOURCE_ICONS[resource.type] || 'fa-file'} className="text-white w-5 h-5 sm:w-6 sm:h-6" /></div>
                  <div className="flex-1 min-w-0">
                    <Breadcrumbs segments={[{ label: t('ialab.breadcrumb_home') }, { label: modules?.find(m => m.id === activeMod)?.title || `Módulo ${activeMod}` }, { label: resource.type?.replace(/_/g, ' ')?.toUpperCase() }]} size="text-xs" className="mb-1 text-slate-400 dark:text-slate-500" />
                    <h2 className="text-lg sm:text-xl font-bold text-petroleum dark:text-white tracking-tight truncate">{resource.title}</h2>
                ...
                {showNotes && (
                  <div className="border-t border-slate-200/60 dark:border-slate-700/60 px-4 sm:px-6 py-4 bg-white dark:bg-slate-800">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${noteSaved ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400' : 'bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setShowNotes(s => !s)}
                      className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg flex items-center gap-1 sm:gap-2 transition-all duration-200 text-sm sm:text-base font-medium border ${
                        showNotes ? 'bg-gradient-to-r from-petroleum to-corporate text-white shadow-sm border-transparent'
                  ...
                  {totalResources > 1 && (
                    <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-center sm:justify-start">
                      <button onClick={onPreviousResource} disabled={currentIndex <= 0}
                        className={cn("px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg flex items-center gap-1 sm:gap-2 transition-all duration-200 text-sm sm:text-base font-medium",
                          currentIndex <= 0 ? "text-slate-300 dark:text-slate-600 cursor-not-allowed opacity-40"
                      ...
                      <button onClick={onNextResource} disabled={currentIndex >= totalResources - 1}
                        className={cn("px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg flex items-center gap-1 sm:gap-2 transition-all duration-200 text-sm sm:text-base font-medium",
                          currentIndex >= totalResources - 1 ? "text-slate-300 dark:text-slate-600 cursor-not-allowed opacity-40"
                            : "bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-petroleum/70 hover:bg-petroleum/5 hover:border-corporate/30 hover:shadow-sm transition-all duration-200")}
