...
const SidebarProgressCircle = ({ courseProgress, t, levelName, streak, xp, atRisk, onCircleClick, onStreakClick }) => {
  ...
  return (
    <div className="flex flex-col items-center px-1">
      <button
        ...
        aria-valuemin="0"
        aria-valuemax="100"
        aria-label={`${pct}% ${t('sidebar.completed') || 'completado'} — ${t('sidebar.toggle_collapse_tip') || 'Haz clic para colapsar o expandir el menú'}`}
      ...
      >
        <svg className="w-full h-full -rotate-90 p-0.5" viewBox="0 0 120 120">
          <circle cx="60" cy="60" r="50" className="stroke-slate-100 dark:stroke-slate-700" strokeWidth="7" fill="none" />
          <circle
            cx="60" cy="60" r="50"
        ...
        <span className="absolute inset-0 flex items-center justify-center">
          <span className="text-center">
            <span className="font-display text-2xl font-extrabold text-petroleum dark:text-[#4DA8C4] leading-none">{pct}%</span>
            <span className="block text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 leading-tight">{t('sidebar.completed')}</span>
          </span>
        </span>
      </button>
      ...
      <div className="flex flex-wrap items-center justify-center gap-1.5 mt-3">
        {levelName && (
          <span className="inline-flex items-center gap-1 rounded-full bg-petroleum/8 dark:bg-petroleum/20 px-2.5 py-1 text-[11px] font-bold text-petroleum dark:text-[#4DA8C4]">
            <Icon name="fa-graduation-cap" className="text-corporate text-[10px]" aria-hidden="true" />
        ...
        {streak > 0 && (
          <button
            ...
            aria-label={`${streak} ${t('sidebar.streak_days', { streak }) || 'días de racha'} — ${t('sidebar.streak_details') || 'Ver detalles de la racha'}`}
          >
            <Icon name="fa-fire" className={atRisk ? 'text-amber-500 text-[10px]' : 'text-orange-500 text-[10px]'} aria-hidden="true" />
            {streak}d
          </button>
        )}
        <span className="inline-flex items-center gap-1 rounded-full bg-petroleum/8 dark:bg-petroleum/20 px-2.5 py-1 text-[11px] font-bold text-petroleum dark:text-[#4DA8C4]">
          <Icon name="fa-star" className="text-corporate text-[10px]" aria-hidden="true" />
          {xp} XP
        </span>
      </div>
    </div>
