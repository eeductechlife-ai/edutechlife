...
const SidebarCollapsed = ({
  ...
  goToProgress, setShowLeaderboard, setShowStudyPlannerModal,
  onToggleSidebar, fadeTransition, t,
}) => (
  <motion.div
    key="collapsed"
    initial={{ opacity: 0 }}
  ...
  >
    ...
    <TooltipIcon decorative label={t('sidebar.completed_pct', { pct: Math.round(courseProgress) })} premium>
      <button
        ...
        aria-valuemin="0"
        aria-valuemax="100"
        aria-label={`${Math.round(courseProgress)}% ${t('sidebar.completed') || 'completado'} — ${t('sidebar.toggle_collapse_tip') || 'Haz clic para expandir el menú'}`}
      >
        <div className="relative w-14 h-14">
          <svg className="w-14 h-14 -rotate-90" viewBox="0 0 120 120">
            ...
            <circle cx="60" cy="60" r="50" className="stroke-slate-200 dark:stroke-slate-700" strokeWidth="10" fill="none" />
            <circle
              cx="60" cy="60" r="50"
              ...
              strokeDashoffset={314.159 - (314.159 * Math.min(courseProgress, 100)) / 100}
    ...
    <TooltipIcon label={t('ialab.tab_progress') || 'Mi Progreso'}>
      <button
        ...
        aria-label={t('ialab.tab_progress') || 'Mi Progreso'}
      >
        <Icon name="fa-chart-line" className="text-white text-xs" aria-hidden="true" />
      </button>
    </TooltipIcon>
    ...
    <div className="flex flex-col items-center gap-1.5 w-full">
      <TooltipIcon decorative label={t('sidebar.level_label', { level: getLevel() })} premium>
        <div className="flex flex-col items-center gap-0">
          ...
          <span className="text-[11px] font-bold text-petroleum dark:text-[#4DA8C4] mt-0.5 leading-tight">{t('sidebar.level', { level: getLevel() })}</span>
        </div>
      </TooltipIcon>
      ...
      <TooltipIcon decorative label={`${t('sidebar.streak_days', { streak })}${isStreakAtRisk() && streak > 0 ? ` — ${t('sidebar.streak_study_today')}` : ''}`} premium>
        <div className="flex flex-col items-center gap-0 relative">
          ...
          >
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center shadow-sm ${isStreakAtRisk() && streak > 0 ? 'bg-gradient-to-br from-amber-400 to-amber-500' : 'bg-gradient-to-br from-orange-400 to-orange-500'}`}>
              <Icon name="fa-fire" className="text-white text-xs" aria-hidden="true" />
            </div>
          </motion.div>
          <span className={`text-[11px] font-bold mt-0.5 leading-tight ${streak >= 3 ? 'text-orange-600' : 'text-slate-500'}`}>{streak}</span>
      ...
      <TooltipIcon decorative label={t('sidebar.points_accumulated', { points: formatPoints(getTotalPoints()) })} premium>
        <div className="flex flex-col items-center gap-0">
          ...
          <span className="text-[11px] font-bold text-petroleum dark:text-[#4DA8C4] mt-0.5 leading-tight">{formatPoints(getTotalPoints())}</span>
        </div>
      </TooltipIcon>
    </div>
    ...
    {/* ── ZONA 3: HERRAMIENTAS ── */}
    <TooltipIcon label={t('ialab.sidebar_study_plan') || 'Plan de Estudio'}>
      <div className="flex flex-col items-center gap-0">
        <button
          ...
          aria-label={t('ialab.sidebar_study_plan') || 'Plan de Estudio'}
        >
          <Icon name="fa-calendar" className="text-white text-xs" aria-hidden="true" />
        </button>
        <span className="text-[10px] font-bold text-petroleum dark:text-[#4DA8C4] mt-0.5 leading-tight">Plan</span>
      </div>
    </TooltipIcon>

    <TooltipIcon label={t('ialab.sidebar_leaderboard') || 'Ranking'}>
      <div className="flex flex-col items-center gap-0">
        <button
        ...
        >
          <Icon name="fa-trophy" className="text-white text-xs" aria-hidden="true" />
        </button>
        <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 mt-0.5 leading-tight">{t('ialab.sidebar_leaderboard') || 'Ranking'}</span>
      </div>
    </TooltipIcon>
