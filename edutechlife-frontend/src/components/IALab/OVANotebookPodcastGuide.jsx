...
export default function OVANotebookPodcastGuide({ onComplete }) {
  ...
  const totalSlides = MODULE_DATA.reduce(
    (acc, mod) => acc + mod.content.length,
    0,
  );
  const currentTotalProgress = Math.min(
    100,
    ((MODULE_DATA.slice(0, currentModuleIndex).reduce(
      (acc, mod) => acc + mod.content.length,
      0,
    ) +
      currentSlide) /
    ...
    if (currentScreen === "modules") {
      ...
      if (slide.items) {
        if (typeof slide.items[0] === "string") text += slide.items.join(". ");
          ...
          slide.items.forEach(
            (item) =>
              (text += `${item.title || item.name}: ${item.desc || item.nb}. `),
          );
      }
      return text;
    }
  ...
  if (currentScreen === "modules") {
    ...
    if (slide.items) {
      if (typeof slide.items[0] === "string")
        textToRead += slide.items.join(". ");
      else
        slide.items.forEach(
          (item) =>
            (textToRead += `${item.title || item.name}: ${item.desc || item.nb}. `),
        );
    }
    ...
    return (
      <SectionErrorBoundary name="OVAPodcastGuide-modules">
        <div
          className="w-full bg-slate-50 dark:bg-slate-800 font-sans flex flex-col relative"
        ...
        >
          ...
          <div className="flex flex-1 flex-col md:flex-row">
            <aside className="w-full md:w-64 bg-white/90 dark:bg-slate-800/90 border-r border-gray-200 dark:border-slate-600 overflow-y-auto">
              <div className="p-4">
                <h3 className="text-sm font-bold text-gray-400 dark:text-slate-400 uppercase tracking-wider mb-4">
                  {t("ova.podcastguide.modules_label")}
                </h3>
            ...
            <main className="flex-1 p-4 md:p-8 overflow-y-auto relative">
              <AnimatePresence mode="wait">
                <motion.div
                  key={`${currentModuleIndex}-${currentSlide}`}
                ...
                >
                        ...
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="bg-gray-50 dark:bg-slate-700/30 text-blue-900 dark:text-slate-100">
                              ...
                              <th className="p-4 border-b-2">
                                {t("ova.podcastguide.comparison_gpt")}
                              </th>
                            </tr>
                          </thead>
                          <tbody>
                            {slide.items.map((item, i) => (
                              ...
                              >
                                <td className="p-4 font-semibold text-gray-700 dark:text-slate-200">
                                  {item.name}
                                </td>
                                <td className="p-4 text-green-600 bg-green-50/30 dark:bg-green-900/20">
                                  {item.nb}
                                </td>
                      ...
                      className={`w-full p-4 text-left rounded-lg border-2 ${FOCUS_RING} font-medium flex items-center justify-between ${activityState === "correct" && opt.correct ? "border-green-500 bg-green-50 dark:bg-green-900/20" : ""} ${activityState === "incorrect" && !opt.correct ? "border-red-500 bg-red-50 dark:bg-red-900/20" : ""} ${!activityState ? "border-gray-200 dark:border-slate-600 hover:border-blue-700 hover:bg-white dark:hover:bg-slate-800 bg-white dark:bg-slate-800" : ""}`}
                        ...
                        {activityState && (
                          <motion.div
                            initial={{ opacity: 0, y: 10 }}
                            animate={{ opacity: 1, y: 0 }}
                            className={`mt-6 p-4 rounded-lg font-medium flex items-start gap-3 ${activityState === "correct" ? "bg-green-100 dark:bg-green-900/30 text-green-800" : "bg-red-100 dark:bg-red-900/30 text-red-800"}`}
                          >
                  ...
                  <div className="mt-10 flex justify-between items-center pt-6 border-t border-gray-100 dark:border-slate-700">
                    <button
                      ...
                      className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold ${FOCUS_RING} ${currentSlide === 0 ? "opacity-0 cursor-default" : "text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700/50"}`}
                    >
                    ...
                    {slide.type !== "activity" ? (
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (currentModuleIndex < MODULE_DATA.length - 1) {
                      ...
                      >
                        {currentModuleIndex < MODULE_DATA.length - 1
                          ...
                          : t("ova.podcastguide.final_challenge")}{" "}
                        <ChevronRight className="w-5 h-5" />
                      </button>
                    )}
                  </div>
            ...
            >
              <div className="text-center mb-8">
                ...
                <p className="text-gray-500 dark:text-slate-400 mt-2">
                  {t("ova.podcastguide.challenge_question", {
                    current: currentSlide + 1,
                    total: FINAL_CHALLENGE.length,
                  })}
                </p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-700/30 p-6 rounded-xl border border-gray-200 dark:border-slate-600 mb-8">
                <p className="text-lg text-gray-800 dark:text-slate-200 font-medium leading-relaxed">
                  {question.question}
                </p>
              </div>
              <div className="space-y-4 mb-8">
                {question.options.map((opt, i) => (
                  ...
                  >
                    ...
                    {activityState === "incorrect" && !opt.correct && (
                      <XCircle className="text-red-500 w-6 h-6 shrink-0" />
                    )}
                  </button>
                ))}
              ...
              <div className="flex justify-end">
                <button
                  ...
                  onClick={() => {
                    if (currentSlide < FINAL_CHALLENGE.length - 1) {
                      setCurrentSlide((s) => s + 1);
                      setActivityState(null);
                ...
                >
                  {currentSlide < FINAL_CHALLENGE.length - 1
                    ...
                    : t("ova.podcastguide.challenge_results")}{" "}
          ...
          <div className="w-full min-h-screen p-4 md:p-8 flex items-center justify-center">
            ...
            >
              ...
              <p className="text-xl text-gray-500 dark:text-slate-400 mb-8">
                {t("ova.podcastguide.cert_subtitle")}
              </p>
              <div className="bg-gray-50 dark:bg-slate-700/30 rounded-2xl p-8 mb-8 border border-gray-100 dark:border-slate-700">
                ...
                <p className="text-gray-600 dark:text-slate-300">
                  {t("ova.podcastguide.cert_message")}
                </p>
              </div>
              <button
                onClick={() => {
