...
// Antes importaba `getModuleContent`, que existe con dos firmas distintas: la de
// selectors.js recibe (moduleId, locale) y la del index recibe (locale). Aquí se
// resolvía a la del index y se le pasaba un número de módulo, así que devolvía
// siempre el mapa completo en español y `content.accordion` (campo que tampoco
// existe: es `accordionContent`) quedaba undefined. Resultado: la búsqueda solo
// encontraba títulos de módulo, nunca temas ni recursos.
import { getModuleOverviewData } from "./constants/moduleContent";
 ...
 */
const GlobalSearchBar = forwardRef(function GlobalSearchBar(
  { mobile, onClose },
  ref,
) {
  ...
  // Estos dos estaban declarados después del `return` de la variante móvil, con
  // lo que el número de hooks cambiaba según el prop `mobile` (violación de las
  // reglas de hooks) y la navegación por teclado quedaba muerta en ese camino.
      ...
      modules.forEach((mod) => {
        ...
        if (modScore > 0) {
          hits.push({
            ...
            id: mod.id,
            label: mod.title,
            sublabel: t("ialab.global_search.module_label", { id: mod.id }),
            modId: mod.id,
          });
        }
        // Los temas de cada módulo viven en overviewData.topics y sus recursos se
        // buscan en el catálogo por título de tema, que es la clave que usa.
        const topics = getModuleOverviewData(mod.id, locale)?.topics || [];
        topics.forEach((topic, topicIdx) => {
          const topicTitle = topic.title || "";
          const topicScore = fuzzyScore(q, topicTitle);
          if (topicScore > 0) {
            hits.push({
              score: topicScore,
              type: "topic",
              id: `mod${mod.id}-${topicIdx}`,
              label: topicTitle,
              sublabel: t("ialab.global_search.topic_label", {
                title: mod.title,
              }),
              modId: mod.id,
            });
          ...
          const topicResources =
            getResourcesForTopic(topicTitle, locale)?.resources || [];
  ...
  if (mobile) {
    return (
      <div
        ...
        className="fixed inset-0 z-[1002] bg-white dark:bg-slate-900 flex flex-col"
      >
        <div className="flex items-center gap-3 p-4 border-b border-slate-200 dark:border-slate-700">
          <button
            onClick={onClose}
            className="w-11 h-11 flex items-center justify-center text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 -ml-1"
            aria-label={t("ialab.global_search.close_aria")}
          >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {query && (
            ...
            >
              {results.length > 0
                ? t("ialab.global_search.results_count", {
                    count: results.length,
                  })
                : t("ialab.global_search.no_results", { query })}
            </p>
          )}
          {results.map((hit, i) => (
            <button
          ...
          {!query && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-sm font-semibold text-slate-500">
                {t("ialab.global_search.empty_title")}
              </p>
              <p className="text-xs text-slate-400 mt-1">
                {t("ialab.global_search.empty_hint")}
              </p>
            </div>
          )}
        </div>
    ...
    >
      <div className="flex items-center">
        ...
        >
          <Icon
            name="fa-search"
            className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 text-xs pointer-events-none"
            aria-hidden="true"
          />
          <input
            ref={inputRef}
      ...
      {isOpen && query && results.length > 0 && (
        <div
          id="search-listbox"
          role="listbox"
          aria-label={t("ialab.global_search.results_label")}
          aria-live="polite"
          aria-atomic="true"
          className="absolute top-full mt-1.5 left-0 w-72 sm:w-80 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl z-[300] overflow-hidden"
        >
          <div className="p-1.5 space-y-0.5">
            {results.map((hit, i) => (
              <button
                ...
                onMouseEnter={() => {
                  setActiveIndex(i);
                  setActiveDescendant(`search-result-${i}`);
                }}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-600 dark:text-slate-500 truncate">
                    {hit.sublabel}
                  </p>
                </div>
              </button>
      ...
      {isOpen && query && results.length === 0 && (
        <div
          aria-live="polite"
          aria-atomic="true"
          className="absolute top-full mt-1.5 left-0 w-72 sm:w-80 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl z-[300] p-4 text-center"
        >
