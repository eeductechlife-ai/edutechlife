...
const Card = ({ children, className = '' }) => (
    <div className={`bg-white/80 backdrop-blur-md border border-white/40 shadow-xl shadow-slate-200/50 rounded-2xl p-6 md:p-8 ${className}`}>
        {children}
    </div>
);
    ...
    return (
        <div className="max-w-4xl mx-auto animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className="col-span-2 bg-[#F8FAFC] border-slate-300">
                    <div className="border-b border-slate-300 pb-3 mb-4 flex items-center gap-2">
                        <span className="font-bold text-slate-800"># claustro-docentes</span>
                    </div>
                            ...
                            <div key={i} className={`flex gap-3 animate-fade-in ${msg.isBot ? 'bg-white p-3 rounded-lg border border-cyan-100 shadow-sm' : ''}`}>
                                ...
                                <div>
                                    <div className="font-bold text-sm text-slate-800 flex items-center gap-2">
                                        {msg.sender}
                                        {msg.isBot && <span className="text-[10px] bg-blue-100 text-blue-800 px-1.5 rounded uppercase">GPT</span>}
                                    </div>
                                    <div className="text-sm text-slate-700 whitespace-pre-line mt-1">{msg.text}</div>
                                </div>
                            </div>
                        ))}
                    </div>
        ...
        <div className="max-w-4xl mx-auto animate-fade-in">
            ...
            <Card className="mb-6">
                ...
                <div className="bg-white rounded-lg border border-slate-200 shadow-inner overflow-hidden">
                    <div className="bg-slate-100 px-4 py-2 border-b text-sm font-medium text-slate-600 flex gap-2">
                        <span>Para: alumnos@instituto.edu</span>
                    </div>
                    <div className="p-4">
                        <textarea
                            className="w-full h-32 p-2 border-none resize-none focus:ring-0 text-slate-700 bg-transparent"
                            value={emailText}
                            readOnly
                        />
                    </div>
                    <div className="bg-slate-50 p-3 border-t flex justify-between items-center">
                        ...
                        {!improved ? (
                            ...
                            <span className="text-green-600 text-sm font-bold flex items-center gap-1"><Check size={16}/> Texto optimizado por GPT</span>
                        )}
                    </div>
                </div>
            </Card>
            ...
            <Card className="min-h-[300px]">
                {tab === 'make' ? (
                    <div className="animate-fade-in">
                        ...
                        <div className="flex items-center justify-center gap-4 bg-slate-50 p-8 rounded-xl border border-slate-200 overflow-x-auto">
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-xs font-bold mt-2 text-slate-600">Pregunta</span>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-xs font-bold mt-2 text-slate-600">API Externa</span>
                            </div>
                            <div className="w-12 h-1 bg-slate-300"></div>
                            <div className="flex flex-col items-center">
                    ...
                    <div className="animate-fade-in">
                        ...
                        <div className="overflow-hidden rounded-xl border border-slate-200">
                            <table className="w-full text-left bg-white">
                                ...
                                <tbody>
                                    <tr>
                                        <td className="p-4 border-b text-slate-700">Dirección</td>
                                        <td className="p-4 border-b text-slate-600">GPT llama a la API</td>
                                        <td className="p-4 border-b text-slate-600">La app avisa al GPT</td>
                                    </tr>
                                    <tr>
                                        <td className="p-4 border-b text-slate-700">Ideal para</td>
                                        <td className="p-4 border-b text-slate-600">Consultar datos externos</td>
                                        <td className="p-4 border-b text-slate-600">Recibir notificaciones</td>
                                    </tr>
                                    <tr>
                                        <td className="p-4 text-slate-700">Ejemplo</td>
                                        <td className="p-4 text-slate-600">Buscar en base de datos</td>
                                        <td className="p-4 text-slate-600">Nuevo formulario enviado</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
        ...
        <div className="max-w-4xl mx-auto animate-fade-in">
            ...
            <Card className="mb-8">
                ...
                {step === 3 && !selections.dest && (
                    <div className="animate-fade-in">
                        <h3 className="text-xl font-bold text-center mb-6 text-slate-700">3. ¿Dónde enviará el resultado?</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {destinations.map(d => (
                                <button key={d.id} onClick={() => handleSelect('dest', d.text)} className="p-6 border-2 border-slate-100 rounded-xl hover:border-purple-500 hover:bg-purple-50 flex flex-col items-center gap-3 transition-all">
                                    <d.icon size={32} className="text-purple-600" />
                                    <span className="font-semibold text-slate-700">{d.text}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                {selections.dest && (
                    <div className="text-center animate-scale-in">
                        ...
                        <p className="mt-4 text-gray-500 text-sm">Así funciona un GPT con Acciones: evento → inteligencia → resultado.</p>
                        <div className="mt-8">
            ...
            {!showResult ? (
                <Card className="mb-8">
                    <div className="flex justify-between items-center mb-6">
                        <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Pregunta {currentQ + 1} de {questions.length}</span>
                    </div>
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        <div className={`w-6 h-6 rounded-full border flex items-center justify-center flex-shrink-0 mt-0.5 ${selected !== null && i === questions[currentQ].answer ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300'}`}>
                ...
                <Card className="text-center animate-scale-in">
                    ...
                    {score === questions.length ? (
                        ...
                        <div className="bg-blue-50 text-blue-800 p-4 rounded-xl mb-8 border border-blue-200">
                            <p className="font-medium">¡Buen trabajo! Repasa los conceptos que te hayan quedado dudosos.</p>
                        </div>
                    ) : (
                        <div className="bg-orange-50 text-orange-800 p-4 rounded-xl mb-8 border border-orange-200">
