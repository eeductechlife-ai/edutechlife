  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-bold text-petroleum uppercase tracking-wider">Analíticas</h2>
      </div>
            ...
            {weeklyData.map((d, i) => {
              ...
              return (
                <div key={d.day} className="flex-1 flex flex-col items-center gap-1 group relative">
                  <div className="w-full rounded-md bg-corporate/5 relative overflow-hidden" style={{ height: 80 }}>
                    <motion.div
                      initial={{ height: 0 }}
          ...
          <div className="space-y-2.5">
            {moduleScores.map(m => (
              <div key={m.id} className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-slate-500 w-10">Mód. {m.id}</span>
                <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${m.score}%` }}
                    transition={{ duration: 0.8, delay: m.id * 0.1, ease: 'easeOut' }}
                    className={`h-full rounded-full ${scoreColor(m.score)}`}
                  />
                </div>
                <span className={`text-[11px] font-bold w-10 text-right ${scoreTextColor(m.score)}`}>{m.score}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-slate-100 dark:border-slate-700">
          ...
          <div className="grid grid-cols-3 gap-2">
            {badgeItems.map(b => (
              ...
              >
                <Icon name={b.icon} className="w-5 h-5 mx-auto mb-1" color={b.earned ? b.color : undefined} />
                <p className={`text-[9px] font-semibold ${b.earned ? 'text-slate-700' : 'text-slate-400'}`}>{b.label}</p>
