...
const actionIcons = {
  ...
  community: { icon: 'fa-comments', bg: 'bg-green-100', color: 'text-green-500' },
  resources: { icon: 'fa-video', bg: 'bg-corporate/10', color: 'text-corporate' },
};
  ...
  const activities = useMemo(() => {
    ...
    for (let id = 5; id >= 1; id--) {
      ...
      if (mod.resourcesCompleted) {
        items.push({ type: 'resources', id: `res-${id}`, label: `Recursos del Módulo ${id} completados`, moduleId: id });
      }
      if (mod.exam) {
        const score = completedExams[id];
        items.push({ type: 'exam', id: `exam-${id}`, label: `Examen Módulo ${id} aprobado${score ? ` (${score}%)` : ''}`, moduleId: id });
      }
      if (mod.challenge) {
        items.push({ type: 'challenge', id: `ch-${id}`, label: `Desafío del Módulo ${id} completado`, moduleId: id });
      }
      if (mod.community) {
        items.push({ type: 'community', id: `comm-${id}`, label: `Participación en foro Módulo ${id}`, moduleId: id });
      }
    }
    ...
    return (
      <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-100">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Actividad Reciente</p>
        </div>
        <p className="text-xs text-slate-400 text-center py-4">No hay actividad registrada aún</p>
      </div>
    );
  }
