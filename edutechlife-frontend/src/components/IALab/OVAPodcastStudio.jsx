...
const StepIndicator = ({ current, total }) => (
  <div className="flex items-center justify-center gap-2 mb-6">
    {Array.from({ length: total }, (_, i) => (
      <div key={i} className={`h-2 rounded-full transition-all duration-500 ${i < current ? 'w-8 bg-corporate' : i === current ? 'w-8 bg-petroleum' : 'w-2 bg-slate-200 dark:bg-slate-600'}`} />
    ))}
  </div>
);
...
export default function OVAPodcastStudio({ onComplete }) {
  ...
  const getValerioText = () => {
    ...
    if (step === 'plan') {
      ...
      return t('ova.podcaststudio.plan_voice', { cTypeLabel: cType?.label, gLabel: g?.label, dCountLabel: dCount?.label });
    }
      ...
      <>
        <div className="ialab-animate-fade-in px-4 py-8">
          <div className="max-w-3xl mx-auto text-center mb-8">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-petroleum dark:text-slate-100 mb-2">{t('ova.podcaststudio.questions_title')}</h2>
          ...
          <div className="max-w-3xl mx-auto space-y-8">
            {showType && (
              <div>
                <h3 className="text-lg font-bold text-petroleum dark:text-slate-100 mb-4 text-center">{t('ova.podcaststudio.q_content_type')}</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {CONTENT_TYPES.map(t => (
                    ...
                    >
                      ...
                      <div className="font-bold text-sm text-petroleum dark:text-slate-100 group-hover:text-corporate transition-colors">{t.label}</div>
            ...
            {showGoal && (
              <div>
                <h3 className="text-lg font-bold text-petroleum dark:text-slate-100 mb-4 text-center">{t('ova.podcaststudio.q_goal')}</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {GOALS.map(g => (
                    ...
                    >
                      ...
                      <div className="font-bold text-sm text-petroleum dark:text-slate-100 group-hover:text-corporate transition-colors">{g.label}</div>
            ...
            {showCount && (
              <div>
                <h3 className="text-lg font-bold text-petroleum dark:text-slate-100 mb-4 text-center">{t('ova.podcaststudio.q_doc_count')}</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {DOC_COUNTS.map(d => (
                    ...
                    >
                      ...
                      <div className="font-bold text-sm text-petroleum dark:text-slate-100 group-hover:text-corporate transition-colors">{d.label}</div>
      ...
      <>
        <div className="ialab-animate-fade-in px-4 py-8">
          <div className="max-w-3xl mx-auto text-center mb-8">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-petroleum dark:text-slate-100 mb-2">{t('ova.podcaststudio.plan_title')}</h2>
          ...
          <div className="max-w-3xl mx-auto space-y-4 mb-8">
            <div className="flex flex-wrap justify-center gap-2">
              <span className="px-4 py-2 bg-cyan-50 text-petroleum dark:text-slate-100 rounded-xl text-sm font-semibold border border-cyan-200">{cType?.icon} {cType?.label}</span>
              <span className="px-4 py-2 bg-purple-50 dark:bg-purple-900/20 text-petroleum dark:text-slate-100 rounded-xl text-sm font-semibold border border-purple-200">{g?.icon} {g?.label}</span>
              <span className="px-4 py-2 bg-amber-50 dark:bg-amber-900/20 text-petroleum dark:text-slate-100 rounded-xl text-sm font-semibold border border-amber-200">{dCount?.icon} {dCount?.label}</span>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-600 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                  ...
                  <div className="text-lg font-bold text-petroleum dark:text-slate-100">{recs.idealSources}</div>
                ...
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                  ...
                  <div className="text-lg font-bold text-petroleum dark:text-slate-100">{recs.estimatedTime}</div>
                ...
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                  ...
                  <div className="text-lg font-bold text-petroleum dark:text-slate-100">{g?.label}</div>
    ...
    <>
      <div className="ialab-animate-fade-in px-4 py-8">
        <div className="max-w-3xl mx-auto text-center mb-8">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-petroleum dark:text-slate-100 mb-2">{t('ova.podcaststudio.checklist_title')}</h2>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-600 shadow-sm mb-6">
            <div className="flex justify-between items-center mb-4">
              <span className="text-sm font-bold text-petroleum dark:text-slate-100">{t('ova.podcaststudio.progress_label')}</span>
              <span className="text-sm font-bold text-corporate">{checkedCount}/{CHECKLIST_ITEMS.length}</span>
            </div>
            <div className="w-full bg-slate-100 dark:bg-slate-700/50 rounded-full h-2.5 overflow-hidden">
              <div className="bg-gradient-to-r from-corporate to-petroleum h-full rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
            </div>
          </div>
