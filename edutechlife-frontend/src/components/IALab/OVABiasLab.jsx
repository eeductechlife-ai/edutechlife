        ...
        <div className="max-w-5xl mx-auto bg-white/85 dark:bg-slate-800/85 backdrop-blur-[20px] border border-corporate/15 shadow-[0_10px_40px_rgba(10,53,80,0.08)] rounded-3xl p-6 md:p-10 min-h-[80vh] flex flex-col relative z-10 border-t-4 border-t-corporate">
          {activeSection === 'intro' && (
            <div className="animate-[fadeIn_0.6s_ease-out_forwards]">
              <h2 className="text-3xl md:text-4xl font-black text-petroleum-dark dark:text-slate-100 mb-6 font-montserrat">{contentData.intro.title}</h2>
              ...
              <div className="mt-6 p-6 bg-corporate/10 border-l-8 border-corporate rounded-r-2xl italic text-petroleum-dark dark:text-slate-100 font-medium">"{contentData.intro.extended}"</div>
            </div>
          )}

          {activeSection === 'cap2' && (
            <div className="animate-[fadeIn_0.6s_ease-out_forwards]">
              <h2 className="text-3xl font-bold text-petroleum-dark dark:text-slate-100 mb-6 font-montserrat">{contentData.cap2.title}</h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
                ...
                <div className="bg-gradient-to-br from-petroleum-dark to-[#0D1B2A] rounded-2xl p-6 text-white shadow-xl flex flex-col justify-center border-b-8 border-corporate">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="p-3 bg-corporate rounded-xl"><Zap size={24}/></div>
                  ...
                  <div className="bg-white/10 rounded-xl p-4 border border-white/20">
                    <p className="text-xs uppercase tracking-widest text-corporate font-bold mb-2">{t('ova.biaslab.case_study')}</p>
          ...
          {(activeSection === 'cap1' || activeSection === 'cap3' || activeSection === 'cap4') && (
            <div className="animate-[fadeIn_0.6s_ease-out_forwards]">
              <h2 className="text-3xl font-bold text-petroleum-dark dark:text-slate-100 mb-6 font-montserrat">{contentData[activeSection].title}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(contentData[activeSection].principles || contentData[activeSection].risks || contentData[activeSection].biases).map((item, idx) => (
                  <div key={idx} className="p-6 bg-slate-50 dark:bg-slate-700/30 rounded-2xl border border-slate-100 dark:border-slate-700 hover:border-corporate transition-all group">
          ...
          {activeSection === 'game' && (
            <div className="flex flex-col h-full items-center justify-center text-center animate-[fadeIn_0.6s_ease-out_forwards]">
              {matchedPairs.length < gameData.length ? (
                <>
                  <div className="mb-8">
                    <h2 className="text-3xl font-black text-petroleum-dark dark:text-slate-100 mb-2 font-montserrat">{t('ova.biaslab.game_title')}</h2>
