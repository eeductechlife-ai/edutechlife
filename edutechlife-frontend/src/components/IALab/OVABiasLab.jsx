...
export default function OVABiasLab({ onComplete }) {
  ...
  useEffect(() => {
    if (selectedCase && selectedConcept) {
      if (selectedCase.id === selectedConcept.id) {
        setMatchedPairs([...matchedPairs, selectedCase.id]);
        ...
        <aside role="dialog" aria-modal="false" aria-label={t("ova.biaslab.sidebar_subtitle")} className="w-full md:w-64 bg-white/90 dark:bg-slate-800/90 flex flex-col shadow-xl z-10 md:min-h-screen border-r border-blue-100">
          <div className="p-6 text-center border-b border-blue-50">
            ...
            <p className="text-[10px] uppercase mt-2 text-slate-600 dark:text-slate-300 font-bold tracking-[0.2em]">
              {t("ova.biaslab.sidebar_subtitle")}
            </p>
          </div>
          ...
          <div className="p-4 border-t border-blue-50">
            <VoiceReader
              text={
                activeSection === "intro"
                  ? contentData.intro.text + " " + contentData.intro.extended
                  : contentData[activeSection]?.text ||
                    t("ova.biaslab.default_voice_text")
              }
            />
          </div>
        </aside>
              ...
              <div className="animate-[fadeIn_0.6s_ease-out_forwards]">
                ...
                <div className="w-full h-64 md:h-80 rounded-2xl mb-8 overflow-hidden shadow-xl border border-[#EAEAEA]">
                  <img
                    ...
                    alt={t("ova.biaslab.intro_img_alt")}
                    loading="lazy"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
                  <div className="space-y-4">
                    <p className="text-base text-slate-600 dark:text-slate-300">
                      {contentData.cap2.text}
                    </p>
                    <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl p-6 border border-emerald-100">
                      <h3 className="font-bold text-emerald-800 mb-4 flex items-center gap-2">
                        <CheckCircle size={18} />{" "}
                        {t("ova.biaslab.responsible_practices")}
                      </h3>
                      <ul className="space-y-3">
                        {contentData.cap2.dos.map((item, i) => (
                          <li
                            key={i}
                            className="flex gap-3 text-emerald-900 dark:text-emerald-100 text-sm bg-white/50 dark:bg-slate-800/50 p-2 rounded-lg border border-emerald-50"
                          >
                            <Zap
                              size={14}
            ...
            {(activeSection === "cap1" ||
              ...
              <div className="animate-[fadeIn_0.6s_ease-out_forwards]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(
                    contentData[activeSection].principles ||
                    contentData[activeSection].risks ||
                    contentData[activeSection].biases
                  ).map((item, idx) => (
                    <div
                      key={idx}
                ...
                {activeSection === "cap3" && (
                  <div className="mt-6 p-6 bg-rose-50 dark:bg-rose-900/20 border-2 border-rose-100 rounded-2xl">
                    <h4 className="text-rose-700 dark:text-rose-300 font-black uppercase text-xs mb-2">
                      {t("ova.biaslab.attention_label")}
                    </h4>
                    <p className="text-rose-900 dark:text-rose-100 font-medium">
                      {contentData.cap3.critical}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col h-full items-center justify-center text-center animate-[fadeIn_0.6s_ease-out_forwards]">
                {matchedPairs.length < gameData.length ? (
                  <>
                    <div className="mb-8">
                      ...
                      <p className="text-slate-500 dark:text-slate-400">
                        {t("ova.biaslab.game_desc")}
                      </p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                      <div className="space-y-3">
                        {shuffledCases.map((item) => (
                          <button
                            key={item.id}
                  ...
                  <div className="p-10 bg-emerald-50 dark:bg-emerald-900/20 rounded-3xl border-4 border-emerald-200 flex flex-col items-center">
                    ...
                    <h2 className="text-3xl font-black text-emerald-900 dark:text-emerald-100 font-montserrat">
                      {t("ova.biaslab.game_complete_title")}
                    </h2>
                    <p className="text-emerald-700 dark:text-emerald-300 mt-2">
                      {t("ova.biaslab.game_complete_desc")}
                    </p>
                    <button
                      onClick={() => {
