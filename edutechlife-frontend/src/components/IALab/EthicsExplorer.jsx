...
const Welcome = ({ onNext, t }) => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-center ialab-animate-fade-in px-4 py-8">
    ...
    <p className="text-lg md:text-xl text-slate-500 font-light mb-2">
      {t("ialab.ethics_explorer.subtitle")}
    </p>
    <p className="text-slate-600 max-w-xl mb-4">
      {t("ialab.ethics_explorer.welcome_desc")}
    </p>
...
export default function EthicsExplorer() {
  ...
  const score =
    evaluatedCount > 0
      ? Math.round(
          Object.values(evaluaciones).reduce((s, v) => {
  ...
  const ejemplos = [
    {
      ...
      texto:
        "Los médicos son más competentes que las enfermeras, según estudios. La mayoría de los líderes empresariales exitosos son hombres porque tienen mayor capacidad de toma de decisiones bajo presión. Las mujeres prefieren carreras en educación y cuidado porque son más empáticas por naturaleza.",
    },
    {
      label: t("ialab.ethics_explorer.example_no_sources"),
      texto:
        "Estudios recientes demuestran que el 94% de las empresas que implementan IA generativa aumentan su productividad en un 300% durante el primer trimestre. Además, el 87% de los empleados reportan mayor satisfacción laboral. Estos números han sido validados por múltiples investigaciones independientes.",
    },
  ];

  if (step === "welcome")
    return <Welcome onNext={() => setStep("paste")} t={t} />;
      ...
      <div className="ialab-animate-fade-in px-4 py-8 max-w-3xl mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-slate-500 text-sm mb-4">
            {t("ialab.ethics_explorer.paste_desc")}
          </p>
        ...
        >
          ...
          {t("ialab.ethics_explorer.apply_filter")}
        </button>
      </div>
    );
  }
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    onClick={() =>
                      setEvaluaciones((prev) => ({ ...prev, [f.id]: "si" }))
                    }
                  ...
                  >
                    <div className="flex items-center justify-center gap-1.5 mb-1">
                      {val === "si" && <SvgCheck className="w-4 h-4" />}{" "}
                      {t("ialab.ethics_explorer.yes")}
                    </div>
                    <div className="text-[9px] font-normal opacity-70 leading-tight">
                      {f.si}
                    </div>
                  </button>
                  <button
                  ...
                  >
                    <div className="flex items-center justify-center gap-1.5 mb-1">
                      {val === "parcial" && <SvgAlert className="w-4 h-4" />}{" "}
                      {t("ialab.ethics_explorer.partial")}
                    </div>
                    <div className="text-[9px] font-normal opacity-70 leading-tight">
                      {f.parcial}
                    </div>
                  ...
                  <button
                    onClick={() =>
                      setEvaluaciones((prev) => ({ ...prev, [f.id]: "no" }))
                    }
                    className={`p-3 rounded-xl border-2 text-xs font-bold transition-all ${
                      val === "no"
                  ...
                  >
                    <div className="flex items-center justify-center gap-1.5 mb-1">
                      {val === "no" && <SvgX className="w-4 h-4" />}{" "}
                      {t("ialab.ethics_explorer.no")}
                    </div>
                    <div className="text-[9px] font-normal opacity-70 leading-tight">
                      {f.no}
                    </div>
      ...
      <div className="ialab-animate-fade-in px-4 py-8 max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-[2rem] p-6 md:p-8 shadow-xl border border-slate-100 mb-6">
          <div className="text-center mb-6">
            ...
            <h3 className={`text-xl font-black ${getScoreColor(score)}`}>
              {getScoreLabel(score, t)}
            </h3>
            <p className="text-slate-500 text-xs mt-1">
              {score >= 80
                ? t("ialab.ethics_explorer.result_good")
                : score >= 50
                  ? t("ialab.ethics_explorer.result_caution")
        ...
        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 mb-6">
          ...
          <p className="text-xs text-slate-500 mb-3">
            {t("ialab.ethics_explorer.reflection_desc")}
          </p>
          <textarea
            value={reflexion}
