...
export default function OVANotebookSimulator({ onComplete }) {
        ...
        <div className="w-full flex items-center justify-center py-12 px-4 relative z-10">
          <div className="w-full max-w-3xl bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-[#2FA8C6]/15 shadow-[0_8px_32px_rgba(31,38,135,0.1)] p-8 md:p-14 rounded-3xl animate-[fadeIn_0.6s_ease-out_forwards] text-center border-t-4 border-t-[#2FA8C6] relative overflow-hidden">
            ...
            <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-[#E0F7FA] text-[#004B63] font-semibold text-[10px] uppercase tracking-[0.15em] border border-[#2FA8C6]/20 mb-6">
              <BrainIcon className="w-3.5 h-3.5" /><span>{t('ova.notebooksim.lab_title')}</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold text-[#1E3A5F] mb-6 tracking-tight font-montserrat">{t('ova.notebooksim.welcome_title')}</h2>
            <div className="bg-[#EAEAEA]/50 dark:bg-slate-700/30 p-6 rounded-2xl mb-6 text-[#1E3A5F] text-base md:text-lg leading-relaxed border border-[#2FA8C6]/20 shadow-inner">
              <p className="mb-4"><strong>{t('ova.notebooksim.welcome_hello')}</strong> {t('ova.notebooksim.welcome_desc_p1')}</p>
              <p>{t('ova.notebooksim.welcome_desc_p2')}</p>
            </div>
            <div className="flex justify-center mb-6"><VoiceReader text={t('ova.notebooksim.welcome_audio')} /></div>
            <button onClick={startGame} className="group relative inline-flex items-center justify-center px-8 py-4 font-bold text-white transition-all duration-300 bg-gradient-to-r from-[#2FA8C6] to-[#1E3A5F] rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:ring-offset-2 focus-visible:ring-offset-[#1E3A5F] hover:scale-105 shadow-[0_10px_20px_rgba(47,168,198,0.3)]" style={{ animation: 'pulseGlow 3s infinite' }}>
              <span className="flex items-center gap-3 text-lg tracking-wide"><NetworkIcon /> {t('ova.notebooksim.start_cta')}</span>
            </button>
          </div>
        </div>
  ...
  if (gameState === 'results') {
    ...
    if (percentage === 100) { message = t('ova.notebooksim.result_perfect_title'); submessage = t('ova.notebooksim.result_perfect_desc'); }
    else if (percentage >= 70) { message = t('ova.notebooksim.result_good_title'); submessage = t('ova.notebooksim.result_good_desc'); }
    else { message = t('ova.notebooksim.result_bad_title'); submessage = t('ova.notebooksim.result_bad_desc'); }
    return (
      <div className="w-full relative" style={{ minHeight: '400px' }}>
        <div className="fixed inset-0 -z-10 opacity-60 bg-[linear-gradient(to_right,#EAEAEA_1px,transparent_1px),linear-gradient(to_bottom,#EAEAEA_1px,transparent_1px)] bg-[length:50px_50px]" /><div className="fixed -top-[15%] -left-[10%] w-[50vw] h-[50vw] -z-10 bg-[radial-gradient(circle,rgba(47,168,198,0.15)_0%,rgba(255,255,255,0)_70%)]" /><div className="fixed -bottom-[20%] -right-[10%] w-[60vw] h-[60vw] -z-10 bg-[radial-gradient(circle,rgba(30,58,95,0.08)_0%,rgba(255,255,255,0)_70%)]" />
        <div className="w-full flex items-center justify-center py-12 px-4 relative z-10">
          <div className="w-full max-w-2xl bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-[#2FA8C6]/15 shadow-[0_8px_32px_rgba(31,38,135,0.1)] p-8 md:p-16 rounded-3xl animate-[fadeIn_0.6s_ease-out_forwards] text-center border-t-4 border-t-[#1E3A5F]">
            ...
            <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 font-semibold text-[10px] uppercase tracking-[0.15em] border border-emerald-200 dark:border-emerald-700 mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5C7 4 7 7 4.5 7.5c-1.5.5-1.5 2.5 0 3.5"/><path d="M10 17.5V19a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-2"/><path d="M20 12.5V19a1 1 0 0 1-1 1h-2"/><path d="M18 4a2 2 0 0 1 2 2"/><path d="M10 4.5V12"/><path d="M2 10.5V12"/><path d="M2 17a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2"/><path d="M20 8.5V12"/></svg><span>{t('ova.notebooksim.completed_label')}</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold mb-2 text-[#1E3A5F] font-montserrat">{t('ova.notebooksim.results_title')}</h2>
            ...
            {score >= 3 && !certCompletedRef.current && (
              <button onClick={() => { certCompletedRef.current = true; onComplete?.(); }}
                ...
                {t('ova.notebooksim.mark_complete')}
              </button>
            )}
            <button onClick={startGame} className="px-8 py-3 rounded-xl bg-white dark:bg-slate-800 text-[#1E3A5F] dark:text-slate-100 border-2 border-[#EAEAEA] dark:border-slate-600 hover:border-[#2FA8C6] hover:text-[#2FA8C6] transition-all font-semibold flex items-center justify-center gap-2 mx-auto shadow-sm">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg>{t('ova.notebooksim.restart')}
            </button>
          </div>
        </div>
        ...
        <div className="w-full py-6 px-4 relative z-10">
          <div className="w-full max-w-5xl mx-auto animate-[fadeIn_0.6s_ease-out_forwards]" key={screen.id}>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="w-full lg:w-2/5 flex flex-col gap-6">
                <div className="relative rounded-2xl overflow-hidden shadow-xl border border-[#EAEAEA] dark:border-slate-600 h-64 lg:min-h-[300px] bg-white dark:bg-slate-800">
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <div className="flex items-center gap-2 text-white text-sm font-semibold bg-[#2FA8C6]/90 w-fit px-4 py-1.5 rounded-lg backdrop-blur-md shadow-lg border border-white/20"><NetworkIcon /> {t('ova.notebooksim.learning_badge')}</div>
                  </div>
                </div>
              ...
              <div className="w-full lg:w-3/5 max-h-[550px] overflow-y-auto">
                <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-[#2FA8C6]/15 shadow-[0_8px_32px_rgba(31,38,135,0.1)] p-6 md:p-10 rounded-3xl flex flex-col border-t-4 border-t-[#2FA8C6]">
                  ...
                  <div className="mb-5">
                    <h3 className="text-xs font-bold text-[#1E3A5F] uppercase tracking-wider mb-3">{t('ova.notebooksim.objectives_title')}</h3>
                  ...
                  <div className="mb-5">
                    <h3 className="text-xs font-bold text-[#1E3A5F] uppercase tracking-wider mb-3">{t('ova.notebooksim.warnings_title')}</h3>
                    <div className="space-y-2">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-[#1E3A5F] uppercase tracking-wider mb-3">{t('ova.notebooksim.example_title')}</h3>
                    <div className="space-y-2">
            ...
            <div className="flex justify-between items-center mt-8 px-2">
              ...
              <button onClick={nextContent} className="px-6 py-3 bg-gradient-to-r from-[#2FA8C6] to-[#1E3A5F] text-white rounded-xl font-bold text-sm shadow-md hover:shadow-lg active:scale-95 transition-all flex items-center gap-2">{isLast ? t('ova.notebooksim.start_quiz') : t('ova.notebooksim.next')} <ArrowRight /></button>
            </div>
            <footer className="mt-8 pt-6 border-t border-[#EAEAEA] dark:border-slate-600 text-center"><p className="text-slate-600 dark:text-slate-400 text-xs">{t('ova.notebooksim.footer')}</p></footer>
          </div>
        </div>
        ...
        <div className="w-full max-w-6xl mx-auto animate-[fadeIn_0.6s_ease-out_forwards]">
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-[#E0F7FA] text-[#004B63] font-semibold text-[10px] uppercase tracking-[0.15em] border border-[#2FA8C6]/20"><BrainIcon className="w-3.5 h-3.5" /><span>{t('ova.notebooksim.lab_title')}</span></div>
              <span className="text-[#1E3A5F] font-semibold bg-white dark:bg-slate-800 px-4 py-1.5 rounded-full border border-[#EAEAEA] dark:border-slate-600 shadow-sm text-sm">{t('ova.notebooksim.node_label', { current: currentQIndex + 1, total: questionsData.length })}</span>
              <span className="text-white font-semibold bg-[#2FA8C6] px-4 py-1.5 rounded-full shadow-[0_0_10px_rgba(47,168,198,0.4)] text-sm">{t('ova.notebooksim.score_label')} {score}</span>
            </div>
          </div>
          <div className="w-full bg-[#EAEAEA] dark:bg-slate-700 rounded-full h-2.5 mb-8 overflow-hidden shadow-inner">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-2/5 flex flex-col gap-6">
              <div className="relative rounded-2xl overflow-hidden shadow-xl border border-[#EAEAEA] dark:border-slate-600 group h-64 lg:h-auto lg:min-h-[300px] bg-white dark:bg-slate-800">
                <img src={currentQ.image} alt={t('ova.notebooksim.image_alt')} loading="lazy" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-90" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#1E3A5F] via-transparent to-transparent opacity-80" />
                <div className="absolute bottom-4 left-4 right-4"><div className="flex items-center gap-2 text-white text-sm font-semibold bg-[#2FA8C6]/90 w-fit px-4 py-1.5 rounded-lg backdrop-blur-md shadow-lg border border-white/20"><NetworkIcon /> {t('ova.notebooksim.scenario_badge')}</div></div>
              </div>
              {isAnswered && (
                <div className={`p-6 rounded-2xl border animate-[fadeIn_0.6s_ease-out_forwards] shadow-sm ${selectedOption === currentQ.correct ? 'bg-[#E8F8F5] dark:bg-emerald-900/20 border-[#22c55e]/40' : 'bg-[#FDEDEC] dark:bg-red-900/20 border-[#ef4444]/40'}`}>
                  <div className="flex items-center gap-3 mb-3">
                    {selectedOption === currentQ.correct
                      ? <span className="flex items-center gap-2 text-[#166534] dark:text-emerald-300 font-bold text-lg"><CheckCircleIcon /> {t('ova.notebooksim.correct_label')}</span>
                      : <span className="flex items-center gap-2 text-[#991b1b] dark:text-red-300 font-bold text-lg"><XCircleIcon /> {t('ova.notebooksim.incorrect_label')}</span>}
                  </div>
              ...
              <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-[#2FA8C6]/15 shadow-[0_8px_32px_rgba(31,38,135,0.1)] p-6 md:p-10 rounded-3xl h-full flex flex-col border-t-4 border-t-[#2FA8C6]">
                ...
                {!isAnswered && (
                  <div className="mt-6 flex flex-col items-start gap-3 animate-[fadeIn_0.6s_ease-out_forwards]">
                    <button onClick={() => setShowHint(!showHint)} className="flex items-center gap-2 px-4 py-2 bg-[#EAEAEA]/40 dark:bg-slate-700/40 hover:bg-[#EAEAEA] dark:hover:bg-slate-600 border border-[#2FA8C6]/30 rounded-xl text-[#1E3A5F] font-semibold text-sm transition-all duration-300"><LightbulbIcon />{showHint ? t('ova.notebooksim.hide_hint') : t('ova.notebooksim.show_hint')}</button>
                    {showHint && (<div className="w-full p-4 bg-[#E8F8F5] dark:bg-emerald-900/20 border border-[#2FA8C6]/50 rounded-xl text-[#1E3A5F] text-sm md:text-base font-medium animate-[fadeIn_0.6s_ease-out_forwards] shadow-inner"><span className="text-[#2FA8C6] font-bold mr-2">{t('ova.notebooksim.hint_prefix')}</span> {currentQ.hint}</div>)}
                  </div>
                )}
                {isAnswered && (
                  <div className="mt-8 flex justify-end animate-[fadeIn_0.6s_ease-out_forwards]">
                    <button onClick={nextQuestion} className="px-8 py-3.5 bg-[#1E3A5F] hover:bg-[#2FA8C6] text-white font-semibold rounded-xl transition-all duration-300 flex items-center gap-3 shadow-lg hover:shadow-[0_8px_20px_rgba(47,168,198,0.4)] hover:-translate-y-1">
                      {currentQIndex === questionsData.length - 1 ? t('ova.notebooksim.process_results') : t('ova.notebooksim.next_node')} <ArrowRight />
                    </button>
                  </div>
                )}
              </div>
