      ...
      {moduleData.topics.map((tema, index) => {
        ...
        >
          <div className="flex flex-col items-center gap-0.5">
            <Icon name={tema.icon} className={`text-xl flex-shrink-0 ${isTopicCompleted ? 'text-emerald-600' : 'text-petroleum'}`} aria-hidden="true" />
          </div>
...
<h4 className={`text-base font-semibold truncate transition-colors duration-300 flex items-center gap-2 font-montserrat ${
      ...
      className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 text-left group/res ${
                     ...
                     <div className="flex items-center justify-center w-8 h-8 flex-shrink-0">
                       <Icon name={resourceLocked ? 'fa-lock' : getResourceIcon(resource.type)} className={`w-4 h-4 ${resourceLocked ? 'text-slate-300' : ''}`} aria-hidden="true" />
                     </div>
                ...
                {(() => {
                  ...
                  if (allDone && isLastTopic) {
                    return (
                      ...
                      >
                        <Icon name="fa-check" className="w-4 h-4" aria-hidden="true" />
                        {t('ialab.completed')}
                      </button>
                    );
                  }
                    ...
                    return (
                      <motion.button
                        ...
                        >
                          ...
                          <Icon name="fa-arrow-right" className="w-4 h-4" aria-hidden="true" />
                        </motion.button>
                      );
                    }
