...
describe("CertificatePreview", () => {
  ...
  test("renders compact version with download button", () => {
    ...
    expect(
      screen.getByText(/ialab\.certificate_preview\.download/i),
    ).toBeInTheDocument();
  });
  ...
  test("renders full certificate view with title", () => {
    ...
    expect(screen.getByText(/CERTIFICADO DE APROBACIÓN/i)).toBeInTheDocument();
    expect(
      screen.getByText(/Se otorga el presente certificado a/i),
    ).toBeInTheDocument();
  });
  ...
  test("shows generating state on download click", async () => {
    ...
    expect(
      screen.getByText(/ialab\.certificate_preview\.generating/i),
    ).toBeInTheDocument();
    await waitFor(() => {
      expect(
        screen.getByText(/ialab\.certificate_preview\.download/i),
