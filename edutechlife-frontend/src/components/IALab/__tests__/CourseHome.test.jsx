...
// Both CourseHomeCompleted and CourseHomeInProgress are lazy-loaded. Mock them
// synchronously so tests don't need to wait for dynamic imports to resolve.
vi.mock("../courseHome/CourseHomeCompleted", () => ({
  default: () => React.createElement("h2", null, "route.course_complete"),
}));

// CourseHomeInProgress reads from the store — import is fine here since the
// store module is already mocked by vi.mock above.
vi.mock("../courseHome/CourseHomeInProgress", async () => {
  const { useIALabStore } = await import("../../../store/ialabStore");
  return {
    default: function MockCourseHomeInProgress() {
...
describe("CourseHome", () => {
  test("renders dashboard when no progress", async () => {
    render(<CourseHome />);
    // CourseHomeInProgress is lazy — findByText waits for the import to resolve.
    expect(
  ...
  test("envuelve el dashboard en data-theme=default con tokens (evita blanco)", async () => {
    const { container } = render(<CourseHome />);
    await screen.findByText("dashboard.continue_learning");
  ...
  test("renders CourseHomeInProgress when user has progress", () => {
    setupStore({
      moduleProgress: {
        1: {
          exam: true,
  ...
  test("renders CourseHomeCompleted when course is completed", async () => {
    setupStore({
      courseProgress: 100,
    ...
    render(<CourseHome />);
    // CourseHomeCompleted is lazy — findByText waits for the import to resolve.
    expect(
      await screen.findByText("route.course_complete"),
    ).toBeInTheDocument();
  });

  test("renders module tabs in CourseHomeInProgress", () => {
    setupStore({
      moduleProgress: {
        1: {
          exam: true,
  ...
  test("renders CourseHomeCompleted when all 5 modules completed by score", () => {
    setupStore({
      moduleProgress: {
        1: {
          exam: true,
