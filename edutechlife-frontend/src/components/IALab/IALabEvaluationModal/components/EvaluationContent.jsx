  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8">
      ...
      <h3 className="text-xl font-bold text-slate-700 mb-2">
        {loadingType === "loading"
          ? t("ialab.evaluation.modal.ai_designing")
          : t("ialab.evaluation.modal.evaluating")}
      </h3>
...
export function ErrorState({ error, onRetry }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8">
      ...
      <h3 className="text-xl font-bold text-slate-700 mb-2">
        {t("ialab.evaluation.modal.load_error_title")}
      </h3>
...
export function StepContent({
  ...
  const ej1Parsed = (() => {
    try {
      return JSON.parse(responses.ej1 || "{}");
    } catch {
      return {};
    }
  })();
  ...
  const researchTopic =
    ...
    exercises?.conceptos?.[0]?.tema ||
    ej1Parsed.topic ||
    currentExercise?.temaInvestigacion ||
    currentExercise?.conceptos?.[0]?.tema ||
    currentExercise?.documentos?.[0]?.tema ||
    "";
  const selectedDocCount = (() => {
    try {
      return JSON.parse(responses.ej1 || "{}").documents?.length || 0;
    } catch {
      return 0;
    }
  })();
    ...
    try {
      ...
      return docs
        .map((d) => {
          ...
          return docObj
            ? { index: d.index, title: docObj.titulo, tipo: docObj.tipo }
            : null;
        })
        .filter(Boolean);
    } catch {
  ...
  const step1Biases = (() => {
    try {
      ...
      return rawBiases.map((b) => ({
        index: b.index,
        label:
          typeof tiposSesgo[b.index] === "string"
            ? tiposSesgo[b.index]
            : tiposSesgo[b.index]?.nombre || `Bias #${b.index}`,
        pipeline: b.pipeline,
      }));
    } catch {
      return [];
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 pb-20">
        ...
        <div className="mb-4 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold text-amber-800">
              {t("ialab.evaluation.modal.offline_title")}
            </p>
            <p className="text-[10px] text-amber-600">
              {t("ialab.evaluation.modal.offline_desc")}
            </p>
          </div>
          <button
      ...
      <div
        ...
        role="region"
        aria-label={t(
          titleKeys[step - 1] || "ialab.evaluation.modal.step1_title",
        )}
      ...
      {formError && (
        <div
          ...
          role="alert"
          aria-live="assertive"
          className="mb-4 px-4 py-3 bg-red-500/90 text-white rounded-xl text-sm font-medium flex items-center gap-2"
        >
