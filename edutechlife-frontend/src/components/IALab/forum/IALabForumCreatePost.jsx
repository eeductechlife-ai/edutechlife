...
import useForumPosts, {
  POST_CATEGORIES,
} from "../../../hooks/IALab/forum/useForumPosts";
...
const IALabForumCreatePost = ({ onClose, onCreated }) => {
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t("ialab.forum.create_post.title")}
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700">
          ...
          <button
            onClick={onClose}
            aria-label="Cerrar"
            className="w-11 h-11 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-500 hover:text-slate-700 transition-colors"
          >
            <Icon name="fa-xmark" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex justify-end mt-1">
              <span
                className={`text-[10px] font-medium ${content.length > 4500 ? "text-amber-500" : "text-slate-600"}`}
              >
                {content.length}/5000
              </span>
            </div>
