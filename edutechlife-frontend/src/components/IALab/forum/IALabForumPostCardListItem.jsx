...
const IALabForumPostCard = ({
  ...
  const categoryStyle =
    CATEGORY_STYLES[post.category] || CATEGORY_STYLES.discussion;
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          >
            <div
              className={`w-9 h-9 rounded-full bg-gradient-to-tr ${getAvatarGradient(profile.full_name)} flex items-center justify-center shadow-sm`}
            >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <IALabForumUserHoverCard userId={post.user_id}>
                <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">
                  {profile.full_name ||
                    post.user_name ||
                    t("ialab.forum.post_card.user_fallback")}
                </span>
            ...
            <div className="flex items-center gap-2 mb-2">
              <span
                className={`px-2 py-0.5 rounded-full text-[9px] font-semibold ${categoryStyle.bg} ${categoryStyle.text}`}
              >
              ...
              {post.tags?.slice(0, 2).map((tag, i) => (
                <span
                  key={i}
                  className="text-[9px] text-slate-600 dark:text-slate-500 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded-full"
                >
                  {tag}
                </span>
              ))}
        ...
        <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-100 dark:border-slate-700/50">
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span>
                {formatCount(
                  voteState?.userVoted ? post.upvotes || 0 : post.upvotes || 0,
                )}
              </span>
            </button>
            ...
            {post.updated_at &&
              new Date(post.updated_at) > new Date(post.created_at) && (
                <span className="text-[9px] text-slate-600">
                  {t("ialab.forum.post_card.edited_label")}
                </span>
              )}
          </div>
          ...
          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            >
              ...
              {shareFeedback && (
                <span className="absolute -top-1 left-1/2 -translate-x-1/2 text-[8px] font-medium text-emerald-600 whitespace-nowrap">
                  {t("ialab.forum.post_card.copied_tooltip")}
                </span>
              )}
            </button>
            <button
              onClick={handleBookmark}
              aria-label={isBookmarked ? "Guardado" : "Guardar"}
              className={`min-w-[44px] min-h-[44px] text-xs transition-colors ${isBookmarked ? "text-amber-500" : "text-slate-600 hover:text-amber-500"}`}
            >
