...
const formatRelativeTime = (dateString, t, locale) => {
  ...
  if (diffMins < 60)
    return t("ialab.forum.notifications.min_ago", { mins: diffMins });
  const diffHours = Math.floor(diffMins / 60);
  if (diffHours < 24)
    return t("ialab.forum.notifications.hour_ago", { hours: diffHours });
  const diffDays = Math.floor(diffHours / 24);
  if (diffDays < 7)
    return t("ialab.forum.notifications.day_ago", { days: diffDays });
              ...
              {notifications.length === 0 ? (
                <div className="text-center py-8 px-4">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {t("ialab.forum.notifications.empty")}
                  </p>
                </div>
              ) : (
