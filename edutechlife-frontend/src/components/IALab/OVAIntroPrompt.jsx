  ...
  return (
    <div className="animate-[fadeIn_0.8s_cubic-bezier(0.16,1,0.3,1)_forwards] space-y-4">
      <div className="p-5 bg-[#F0F9FF] rounded-[2rem] border-2 border-white dark:border-slate-700 shadow-md">
        <h4 className="text-petroleum font-[900] text-xl mb-3 tracking-tighter lowercase">{t('ova.introprompt.whatis_title')}</h4>
    ...
    <div className="animate-[fadeIn_0.8s_cubic-bezier(0.16,1,0.3,1)_forwards] space-y-3">
        ...
        {types.map(tp => (
          <button key={tp.k} onClick={() => setSel(tp)} className={`p-4 rounded-xl border-2 transition-all text-left ${sel?.k === tp.k ? 'border-corporate bg-blue-50 shadow-md' : 'bg-white dark:bg-slate-800 border-slate-50 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600'}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h5 className="font-[900] text-petroleum text-[11px] uppercase leading-none mb-1">{labels[tp.k]}</h5>
      ...
      {sel && (
        <div className="p-4 bg-petroleum text-white rounded-xl">
          <p className="text-xs font-medium"><span className="text-corporate font-black uppercase text-[10px]">{labels[sel.k]}:</span> {t(`ova.introprompt.types_desc_${sel.k}`)}</p>
        </div>
      )}
    </div>
  );
...
const EffectiveAnatomy = () => {
  ...
  const [sel, setSel] = useState(null);
  const elements = [
    { k: 'rol', c: 'bg-petroleum', i: '👤' },
    { k: 'contexto', c: 'bg-corporate', i: '🌍' },
  ...
  return (
    <div className="animate-[fadeIn_0.8s_cubic-bezier(0.16,1,0.3,1)_forwards] space-y-3">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
        {elements.map(el => (
          <button key={el.k} onClick={() => setSel(el)} className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center gap-1.5 group ${sel?.k === el.k ? 'border-corporate bg-blue-50 shadow-md' : 'bg-white dark:bg-slate-800 border-slate-50 dark:border-slate-700 hover:border-slate-200'}`}>
            ...
            <span className="text-[8px] font-black text-petroleum uppercase tracking-tighter leading-none">{t(`ova.introprompt.anatomy_${el.k}`)}</span>
          </button>
        ))}
      </div>
      {sel ? (
        <div className="p-4 bg-petroleum text-white rounded-xl">
          <h5 className="text-corporate font-[900] text-[10px] uppercase tracking-[0.2em] mb-2">{t(`ova.introprompt.anatomy_${sel.k}`)}</h5>
    ...
    <div className="animate-[fadeIn_0.8s_cubic-bezier(0.16,1,0.3,1)_forwards] space-y-4">
      <div className="p-5 bg-gradient-to-br from-green-50 to-white rounded-[2rem] border-2 border-green-100 dark:border-slate-700 shadow-md">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h4 className="text-petroleum font-[900] text-xl tracking-tighter lowercase">{t('ova.introprompt.strategies_title')}</h4>
        </div>
        <p className="text-slate-600 dark:text-slate-300 leading-relaxed font-bold text-sm">{text}</p>
      </div>
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={s.t} className="p-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl shadow-sm">
            ...
            <h5 className="font-[900] text-petroleum text-[9px] uppercase tracking-wider mb-0.5">{s.t}</h5>
            <p className="text-[9px] text-slate-500 dark:text-slate-400 leading-relaxed">{s.d}</p>
          </div>
        ))}
      </div>
    </div>
...
export default function OVAIntroPrompt({ onComplete }) {
  ...
  const getValerioText = () => {
    switch (screen) {
      case 'welcome': return t('ova.introprompt.welcome_audio');
      case 'm1': return t('ova.introprompt.whatis_desc');
      case 'm2': return t('ova.introprompt.importance_desc');
      case 'm3': return t('ova.introprompt.types_desc_abierto');
      case 'm4': return t('ova.introprompt.anatomy_rol_desc');
      case 'm5': return t('ova.introprompt.universal_desc');
      case 'm6': return t('ova.introprompt.strategies_desc');
      case 'm7': return t('ova.introprompt.errors_vagos');
      case 'm8': return t('ova.introprompt.conclusion_desc');
      default: return '';
    }
  };
        ...
        <div className="flex items-center gap-4">
          {screen !== 'welcome' && (
            <div className="flex items-center gap-1.5 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded-full border border-corporate/20">
              ...
              <span className="font-bold text-petroleum text-[11px]">{nav.filter(id => completed.includes(id)).length}/{nav.length - 1}</span>
            </div>
          )}
        ...
        <div className="w-full bg-white dark:bg-slate-800 rounded-2xl shadow-md p-4 md:p-6 relative overflow-hidden border border-slate-50 dark:border-slate-700">
          {screen.startsWith('m') && (
            <div className="mb-4 border-b border-slate-50 dark:border-slate-700 pb-3">
              <div className="flex items-center gap-1.5 text-corporate font-[900] text-[8px] tracking-[0.3em] uppercase"><Sparkles className="w-3 h-3" /> {t('ova.introprompt.master')}</div>
      ...
      {screen !== 'welcome' && (
        <div className="flex justify-center border-t border-slate-100 dark:border-slate-700 bg-white/90 dark:bg-slate-800/90">
          <div className="w-full max-w-4xl flex justify-between items-center gap-3 px-4 py-3">
            ...
            <button onClick={nextScreen} className={`px-6 min-h-[44px] rounded-xl font-[900] text-[11px] shadow-md active:scale-95 transition-all flex items-center gap-2 uppercase tracking-[0.15em] ${isLastScreen ? 'bg-emerald-500 text-white hover:bg-emerald-600' : 'bg-gradient-to-r from-petroleum to-corporate text-white'}`}>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 dark:bg-slate-950/60 backdrop-blur-md" onClick={() => setIsMenuOpen(false)}>
          <div className="absolute right-0 h-full w-[260px] bg-white dark:bg-slate-800 shadow-2xl p-5 flex flex-col gap-3 animate-[slideInRight_0.5s_cubic-bezier(0.16,1,0.3,1)_forwards]" onClick={e => e.stopPropagation()}>
            <button onClick={() => setIsMenuOpen(false)} aria-label="Cerrar menú" className="self-end min-w-[44px] min-h-[44px] p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full"><X className="w-5 h-5 text-slate-600 dark:text-slate-300" /></button>
            ...
            {nav.map(id => (
              <button key={id} onClick={() => goToScreen(id)} className={`p-3 rounded-lg text-left text-[10px] font-[900] transition-all flex items-center justify-between ${screen === id ? 'bg-petroleum text-white shadow-lg' : 'hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-300'}`}>
                <span className="uppercase tracking-wider">{id === 'welcome' ? t('ova.introprompt.menu_welcome') : screensData[id]?.title}</span>
                {completed.includes(id) && <CheckCircle2 className="w-3 h-3 text-corporate" />}
              </button>
            ))}
          </div>
        </div>
