...
const CourseCard = ({ course, isSignedIn }) => {
  ...
  const getButtonText = (status, signedIn) => {
    const map = {
      active: signedIn
        ? t("ialab.course_card.btn_start")
        : t("ialab.course_card.btn_enroll"),
  ...
  return (
    <ErrorBoundary>
      <motion.div
        variants={fadeInUp}
        className="group relative bg-white dark:bg-slate-800/80 border border-[var(--theme-emphasis)]/10 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 h-fit"
      ...
      >
        <motion.div
          ...
          style={{
            padding: "2px",
            background:
              "linear-gradient(135deg, #004B63, #00BCD4, #66CCCC, #004B63)",
        ...
        <div
          className={`relative h-44 bg-gradient-to-br ${config.bg} p-5 flex flex-col justify-between`}
        >
          <div className="absolute inset-0 opacity-15">
            <motion.div
              ...
              animate={
                shouldReduceMotion ? {} : { x: [0, 15, 0], y: [0, -15, 0] }
              }
          ...
          <div className="flex items-start justify-between relative z-10">
            <div
              className={`px-2.5 py-1 rounded-md text-[10px] md:text-[11px] font-bold uppercase tracking-wider ${config.badge}`}
            >
          ...
          <div className="relative z-10 flex items-center gap-2 text-white/70 text-[11px] md:text-xs">
            ...
            <span>
              {t("ialab.course_card.students", { count: course.students })}
            </span>
          </div>
        </div>
        ...
        <div className="p-4 flex-1 flex flex-col">
          <div className="flex flex-wrap gap-1.5 mb-3">
            {course.features.map((f, i) => (
              <span
                key={i}
                className="px-2 py-0.5 bg-[var(--theme-emphasis)]/5 text-[var(--theme-emphasis)] text-[10px] md:text-xs font-semibold rounded-md border border-[var(--theme-emphasis)]/10"
              >
                {f}
              </span>
            ))}
          ...
          {course.status === "active" && course.progress > 0 && (
            <div className="mb-3">
              <div className="flex items-center justify-between text-[10px] md:text-xs text-slate-500 dark:text-slate-400 mb-1">
                ...
                <span className="font-semibold text-[var(--theme-emphasis)]">
                  {course.progress}%
                </span>
              </div>
          ...
          >
            ...
            <span className="relative">
              {getButtonText(course.status, isSignedIn)}
            </span>
            {!shouldReduceMotion && (
              <motion.span
                className="relative"
