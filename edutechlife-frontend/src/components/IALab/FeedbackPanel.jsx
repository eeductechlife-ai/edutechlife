        ...
        <div key={index} className="bg-white border border-slate-200 dark:bg-slate-800 dark:border-slate-700 rounded-2xl p-6">
          <div className="flex items-center gap-4 mb-4">
            <div className={`w-12 h-12 rounded-xl ${config.bgColor} flex items-center justify-center`}>
              <Icon name={config.icon} className={`${config.color} text-lg`} />
            </div>
            <div className="flex-1">
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{t(config.titleKey)}</h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm">{t('ialab.evaluation.results.detailed_analysis')}</p>
            </div>
            <div className={`px-4 py-2 rounded-lg text-lg font-bold ${nota >= 80 ? 'bg-emerald-50 text-emerald-600' : nota >= 60 ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-600'}`}>
              {nota}%
            </div>
          </div>
          <div className="mb-3">
