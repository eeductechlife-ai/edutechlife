...
const VoiceReader = ({ text }) => {
  ...
  return (
    <button onClick={speak} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${isPlaying ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-[#E0F7FA] text-[var(--theme-emphasis)] hover:bg-[#B2EBF2]'}`} title={tInner('ialab.interaction_advisor.tool_aria')}>
      ...
      <div onClick={() => setIsOpen(!isOpen)} className="flex items-center justify-between cursor-pointer group">
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h2 className="text-lg md:text-xl font-bold text-[var(--theme-emphasis)] group-hover:text-[var(--theme-primary)] transition-colors duration-300">{t('ialab.interaction_advisor.title')}</h2>
          </div>
        ...
        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ml-4 transition-all duration-300 ${isOpen ? 'bg-[var(--theme-emphasis)]/10 rotate-180' : 'bg-[var(--theme-primary)]/15 group-hover:scale-110'}`}>
          <Icon name="fa-chevron-down" className={`w-4 h-4 ${isOpen ? 'text-[var(--theme-emphasis)]' : 'text-[var(--theme-primary)]'} transition-colors`} />
        </div>
      </div>

      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
          <div className="bg-gradient-to-br from-[#F0F9FF] to-[#E0F7FA] p-5 rounded-xl border border-[var(--theme-primary)]/10">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Icon name="fa-wand-magic-sparkles" className="w-5 h-5 text-[var(--theme-primary)]" />
                <h3 className="font-bold text-[var(--theme-emphasis)] text-sm">{t('ialab.interaction_advisor.question')}</h3>
              </div>
            ...
            <div className="relative">
              <textarea value={input} onChange={handleInputChange} onKeyDown={handleKeyDown} placeholder={t('ialab.interaction_advisor.input_placeholder')} aria-label={t('ialab.interaction_advisor.question')} className="w-full min-h-[80px] p-4 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[var(--theme-primary)]/30 focus:border-[var(--theme-primary)] resize-none transition-all" maxLength={500} />
              <div className="absolute bottom-3 right-3 text-[10px] text-slate-600 font-medium">{input.length}/500</div>
            </div>
            <div className="flex justify-end mt-3">
              <button onClick={handleRecommend} disabled={input.trim().length < 3} className="px-6 py-2.5 bg-gradient-to-r from-[var(--theme-emphasis)] to-[var(--theme-primary)] text-white text-sm font-bold rounded-xl hover:from-[var(--theme-emphasis)]-dark hover:to-[var(--theme-primary)]-dark transition-all duration-200 shadow-sm hover:shadow flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {suggestions.map((s, i) => (
                <button key={i} onClick={() => handleSuggestion(s.text)} className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-50 hover:bg-[#E0F7FA] border border-slate-200 hover:border-[var(--theme-primary)]/30 rounded-lg text-xs font-medium text-slate-600 hover:text-[var(--theme-emphasis)] transition-all">
                  <Icon name={s.icon} className="w-3.5 h-3.5 text-[var(--theme-primary)]" /> {s.label}
                </button>
              ))}
            </div>
          </div>
          ...
          {showResult && recommendation && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="border-t border-slate-100 pt-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-[var(--theme-emphasis)] text-sm flex items-center gap-2"><Icon name="fa-circle-check" className="w-4 h-4 text-emerald-500" /> {t('ialab.interaction_advisor.recommended_tool')}</h3>
              ...
              <div className="bg-gradient-to-br from-[var(--theme-emphasis)] to-[var(--theme-emphasis)]-dark p-5 rounded-xl text-white">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 bg-white/15 rounded-lg flex items-center justify-center backdrop-blur-sm">
                    <Icon name={recommendation.tool.icon} className="w-5 h-5 text-[var(--theme-primary)]" />
                  </div>
                  <div>
              ...
              <div className="bg-[#F0FDFF] p-4 rounded-xl border border-[var(--theme-primary)]/10">
                <h4 className="font-bold text-[var(--theme-emphasis)] text-xs mb-2 flex items-center gap-1.5"><Icon name="fa-lightbulb" className="w-3.5 h-3.5" /> {t('ialab.interaction_advisor.suggested_prompt')}</h4>
