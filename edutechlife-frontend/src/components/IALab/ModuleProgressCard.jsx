...
const ModuleProgressCard = memo(
  ({
    ...
    const moduleLessons = getAllLessons(locale)?.[moduleId] || [];
    const completedModuleLessons = Object.values(moduleLessonProgress).filter(
      (s) => s === "completed",
    ).length;
      ...
      <div className="flex items-start gap-3 p-4 rounded-xl bg-white border border-slate-200/60 shadow-sm hover:shadow-md hover:border-slate-300/50 transition-all duration-200 active:scale-[0.99] group">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            <span
              className={`text-xs font-bold flex-shrink-0 ${isPassed ? "text-emerald-600" : score >= 60 ? "text-amber-600" : "text-slate-500"}`}
            >
              {Math.round(score)}%
            </span>
          </div>
          <div className="flex items-center gap-1.5 flex-wrap mb-2.5">
            <ResourceBadge
              completed={moduleVideos}
              total={config.videos}
              icon="fa-play-circle"
            />
            <ResourceBadge
              completed={moduleInfographics}
              total={config.infographics}
              icon="fa-file-image"
            />
            {examScore > 0 && (
              <span
                className={`inline-flex items-center gap-[3px] text-[9px] font-bold px-1.5 py-0.5 rounded-md border ${
                  examScore >= 80
                    ? "bg-emerald-50 text-emerald-600 border-emerald-200"
                    : "bg-amber-50 text-amber-600 border-amber-200"
                }`}
              >
                <Icon name="fa-file-alt" className="text-[7px]" />{" "}
                {t("ialab.module_progress_card.exam_abbr")}
                {examScore}%
              </span>
            )}
            {challengeScore > 0 && (
              ...
              >
                <Icon name="fa-trophy" className="text-[7px]" />{" "}
                {t("ialab.module_progress_card.challenge_abbr")}
                {challengeScore}%
              </span>
            )}
            {completedModuleLessons > 0 && (
              <span className="inline-flex items-center gap-[3px] text-[9px] font-bold px-1.5 py-0.5 rounded-md border bg-emerald-50 text-emerald-600 border-emerald-200">
                <Icon name="fa-check-circle" className="text-[7px]" />{" "}
                {t("ialab.module_progress_card.lesson_abbr")}
