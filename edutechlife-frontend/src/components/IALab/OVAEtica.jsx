...
const VoiceReader = ({ text }) => {
  ...
  return (
    <button onClick={speak} className={`flex items-center gap-3 px-6 py-3 rounded-[2.5rem] font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-xl ${isPlaying ? 'bg-red-600 text-white shadow-red-600/30' : 'bg-[#F1F5F9] text-[#0D2B5B] hover:bg-slate-200'}`} title="Escuchar con voz de Valerio">
...
const WelcomeScreen = ({ onNext }) => (
  <div className="text-center space-y-4 py-4 animate-in fade-in duration-1000">
    ...
    <p className="text-slate-500 max-w-lg mx-auto font-bold text-[13px]">Hola, soy Valerio, tu coach de IA. En este laboratorio exploraremos los orígenes de la IA generativa y dominaremos el arte de la ingeniería de prompts.</p>
...
const ModuleHistory = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="p-5 bg-white border border-slate-100 rounded-xl shadow-md relative overflow-hidden flex flex-col justify-center min-h-[150px]">
...
const PromptConcept = () => (
  <div className="space-y-4 animate-in fade-in">
    <div className="p-5 bg-[#F0F9FF] rounded-[2rem] border-2 border-white shadow-md relative overflow-hidden">
      <h4 className="text-[#0D2B5B] font-[900] text-xl mb-3 tracking-tighter leading-none lowercase">el prompt como interfaz</h4>
      <p className="text-slate-600 leading-relaxed font-bold text-sm max-w-2xl">Es la interfaz de comunicación entre el usuario y un modelo de lenguaje. Es el conjunto de instrucciones que sirven de puente entre la mente humana y la red neuronal de la IA. Su diseño determina la calidad de los resultados.</p>
    ...
    <div className="bg-white border border-slate-100 p-4 rounded-[1.5rem] flex items-center gap-3 shadow-sm">
      <div className="p-3 bg-amber-50 rounded-xl text-amber-500 shadow-inner"><Lightbulb size={24} /></div>
      <p className="text-[11px] text-slate-500 font-bold italic leading-relaxed">"Dominarás qué es un prompt, sus tipos y por qué su calidad determina directamente la calidad de la respuesta de la IA."</p>
    </div>
  </div>
);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center animate-in zoom-in">
      <div className="grid grid-cols-3 gap-2">
        {elements.map((el) => (
          <button key={el.k} onClick={() => setSel(el)} className={`p-4 rounded-[1.8rem] border-2 transition-all flex flex-col items-center gap-2 group ${sel?.k === el.k ? 'border-[#00B4D8] bg-blue-50 shadow-md' : 'bg-white border-slate-50 hover:border-slate-200'}`}>
  ...
  if (showResult) {
    return (
      <div className="text-center py-4 animate-in zoom-in">
        ...
        <p className="text-slate-500 mt-4 font-bold text-sm">{score === 5 ? '¡Puntuación perfecta! Valerio está orgulloso de ti.' : score >= 3 ? '¡Buen trabajo! Repasa los conceptos dudosos.' : 'No te preocupes, repasa el laboratorio. Valerio confía en ti.'}</p>
...
const CertificateScreen = ({ xp }) => (
  <div className="mx-auto animate-fade-in text-center">
    ...
    <p className="text-sm text-gray-600 mb-4">Has completado el laboratorio guiado por Valerio. Ahora comprendes los orígenes de la IA generativa y el arte de la ingeniería de prompts.</p>
    <div className="bg-gradient-to-br from-[#0D2B5B] to-[#0A1F3F] p-[2px] rounded-2xl shadow-lg mx-auto max-w-md transform hover:scale-105 transition-transform duration-500">
      <div className="bg-white rounded-[14px] p-5 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-20 h-20 bg-cyan-100 rounded-bl-full opacity-50"></div>
        <div className="absolute bottom-0 left-0 w-20 h-20 bg-slate-100 rounded-tr-full opacity-50"></div>
        ...
        <h3 className="text-base font-bold text-slate-800 mb-3">Laboratorio: Comienzos de la IA</h3>
        <div className="flex justify-center gap-4 text-slate-600 border-t border-b border-slate-100 py-2 mb-3 text-xs">
    ...
    <div className="flex flex-col sm:flex-row justify-center gap-3 mt-4">
      <Button onClick={() => window.location.reload()} className="bg-transparent text-[#0D2B5B] border-2 border-[#0D2B5B] hover:bg-slate-50 text-xs">Reiniciar</Button>
...
export default function OVAEtica() {
  ...
  const renderContent = () => {
    switch (screen) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-in slide-in-from-bottom">
            {nav.slice(2).map((id) => (
              <button key={id} onClick={() => setScreen(id)} className="group bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-lg hover:border-[#00B4D8] transition-all flex flex-col items-center text-center gap-4 relative overflow-hidden">
                <div className="bg-[#F0F9FF] text-[#0D2B5B] p-4 rounded-[1.5rem] shadow-sm group-hover:scale-110 transition-transform"><BookOpen className="w-6 h-6" /></div>
      ...
      case 'certificate': return <CertificateScreen xp={xp} />;
      default: return null;
    }
  };
    ...
    <div className="w-full bg-[#F8FAFC] text-slate-900 font-sans flex flex-col selection:bg-blue-100">
      <header className="sticky top-0 w-full bg-white/80 backdrop-blur-2xl border-b z-50 px-4 py-3 flex justify-between items-center shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 bg-[#F1F5F9] hover:bg-slate-200 rounded-xl transition-all shadow-sm border border-slate-100"><Menu className="w-5 h-5 text-[#0D2B5B]" /></button>
        </div>
      </header>
      ...
      <main className="flex-1 px-3 py-4">
        <div className="w-full bg-white rounded-2xl shadow-md p-4 md:p-6 relative overflow-hidden border border-slate-50">
          {screen.startsWith('m') && (
            <div className="mb-4 flex flex-col md:flex-row justify-between items-start md:items-end gap-2 border-b border-slate-50 pb-4">
              <div className="space-y-1">
      ...
      <style>{`
        ...
        .slide-in-from-right { animation: slideInRight 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
        .zoom-in { animation: zoomIn 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
      `}</style>
    </div>
  );
}
