...
import {
  ...
  ArrowRight, ArrowLeft, Award, Play
} from 'lucide-react';
...
export default function OVANotebookBase({
  ...
  const getValerioText = () => {
    ...
    if (gameState === 'content') return contentScreens[contentIdx]?.valerioText || '';
    if (gameState === 'quiz') return questionsData[currentQIndex]?.question || '';
  ...
  if (gameState === 'results') {
    ...
    if (percentage === 100) { message = tk('result_perfect_title'); submessage = tk('result_perfect_desc'); }
    else if (percentage >= 70) { message = tk('result_good_title'); submessage = tk('result_good_desc'); }
    else { message = tk('result_bad_title'); submessage = tk('result_bad_desc'); }
    return (
      <div className="w-full relative min-h-[400px]">
        ...
        <div className="w-full flex items-center justify-center py-12 px-4 relative z-10">
          <div className="w-full max-w-2xl bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-corporate/15 shadow-[0_8px_32px_rgba(31,38,135,0.1)] p-8 md:p-16 rounded-3xl animate-[fadeIn_0.6s_ease-out_forwards] text-center border-t-4 border-t-petroleum">
            ...
            <h2 className="text-3xl md:text-4xl font-bold mb-2 text-petroleum dark:text-slate-100 font-montserrat">{tk('results_title')}</h2>
          ...
          <div className="w-full max-w-5xl mx-auto animate-[fadeIn_0.6s_ease-out_forwards]" key={screen_data.id}>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="w-full lg:w-2/5 flex flex-col gap-6">
                <div className={`relative rounded-2xl overflow-hidden shadow-xl border border-gray-200 dark:border-slate-600 h-64 ${imageLayoutClass} bg-white dark:bg-slate-800`}>
              ...
              <div className={`w-full lg:w-3/5 ${contentPanelMaxHeight} overflow-y-auto`}>
                <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-corporate/15 shadow-[0_8px_32px_rgba(31,38,135,0.1)] p-6 md:p-10 rounded-3xl flex flex-col border-t-4 border-t-corporate">
                  ...
                  <div className="mb-5">
                    <h3 className="text-xs font-bold text-petroleum dark:text-slate-100 uppercase tracking-wider mb-3">{tk('objectives_title')}</h3>
                  ...
                  <div className="mb-5">
                    <h3 className="text-xs font-bold text-petroleum dark:text-slate-100 uppercase tracking-wider mb-3">{tk('warnings_title')}</h3>
                    <div className="space-y-2">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-petroleum dark:text-slate-100 uppercase tracking-wider mb-3">{tk('example_title')}</h3>
                    <div className="space-y-2">
            ...
            <div className="flex justify-between items-center mt-8 px-2">
              ...
              <button onClick={nextContent}
                ...
                {isLast ? tk('start_quiz') : tk('next')} <ArrowRight size={18} />
              </button>
            </div>
            <footer className="mt-8 pt-6 border-t border-gray-200 dark:border-slate-600 text-center">
              <p className="text-slate-600 dark:text-slate-300 text-xs">{tk('footer')}</p>
            </footer>
          </div>
        </div>
        <OVAValerioBar text={getValerioText()} />
    ...
    <div className="w-full relative min-h-[400px]">
      ...
      <div className="w-full py-6 px-4 relative z-10" key={currentQ.id}>
        <div className="w-full max-w-6xl mx-auto animate-[fadeIn_0.6s_ease-out_forwards]">
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-cyan-50 dark:bg-cyan-900/30 text-corporate dark:text-teal-200 font-semibold text-[10px] uppercase tracking-[0.15em] border border-corporate/20">
                <Brain size={14} /><span>{tk('lab_title')}</span>
              </div>
              <span className="text-petroleum dark:text-slate-100 font-semibold bg-white dark:bg-slate-800 px-4 py-1.5 rounded-full border border-gray-200 dark:border-slate-600 shadow-sm text-sm">
                {tk('node_label', { current: currentQIndex + 1, total: questionsData.length })}
              </span>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-2/5 flex flex-col gap-6">
              <div className={`relative rounded-2xl overflow-hidden shadow-xl border border-gray-200 dark:border-slate-600 group h-64 ${imageLayoutClass} bg-white dark:bg-slate-800`}>
                <img src={currentQ.image} alt={tk('image_alt')} loading="lazy" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-90" />
                <div className="absolute inset-0 bg-gradient-to-t from-petroleum via-transparent to-transparent opacity-80" />
                <div className="absolute bottom-4 left-4 right-4">
                  <div className="flex items-center gap-2 text-white text-sm font-semibold bg-corporate/90 w-fit px-4 py-1.5 rounded-lg backdrop-blur-md shadow-lg border border-white/20">
                ...
                <div className={`p-6 rounded-2xl border animate-[fadeIn_0.6s_ease-out_forwards] shadow-sm ${selectedOption === currentQ.correct ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-500/40' : 'bg-red-50 dark:bg-red-900/30 border-red-400/40'}`}>
                  <div className="flex items-center gap-3 mb-3">
                    {selectedOption === currentQ.correct ? (
                      <span className="flex items-center gap-2 text-emerald-700 dark:text-emerald-300 font-bold text-lg"><CheckCircle size={24} /> {tk('correct_label')}</span>
                    ) : (
                      <span className="flex items-center gap-2 text-red-700 dark:text-red-300 font-bold text-lg"><XCircle size={24} /> {tk('incorrect_label')}</span>
                    )}
                  </div>
                ...
                {!isAnswered && (
                  <div className="mt-6 flex flex-col items-start gap-3 animate-[fadeIn_0.6s_ease-out_forwards]">
                    <button onClick={() => setShowHint(!showHint)}
                      ...
                      <Lightbulb size={16} />{showHint ? tk('hide_hint') : tk('show_hint')}
                    </button>
                    {showHint && (
                      <div className="w-full p-4 bg-emerald-50 dark:bg-emerald-900/30 border border-corporate/50 rounded-xl text-petroleum dark:text-slate-100 text-sm md:text-base font-medium animate-[fadeIn_0.6s_ease-out_forwards] shadow-inner">
                        <span className="text-corporate font-bold mr-2">{tk('hint_prefix')}</span> {currentQ.hint}
                      </div>
                    )}
                  </div>
                )}
                {isAnswered && (
                  <div className="mt-8 flex justify-end animate-[fadeIn_0.6s_ease-out_forwards]">
                    <button onClick={nextQuestion}
                      ...
                      {currentQIndex === questionsData.length - 1 ? tk('process_results') : tk('next_node')} <ArrowRight size={18} />
                    </button>
                  </div>
                )}
              </div>
