...
const VoiceReader = ({ text }) => {
  ...
  return (
    <button onClick={speak} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors shrink-0 ${isPlaying ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-[#2596be] text-white hover:bg-[#1f7e9f] shadow-md'}`} title="Escuchar con voz de Valerio">
      ...
      <button onClick={() => setIsExpanded(!isExpanded)} className="w-full flex items-start gap-4 p-4 md:p-5 text-left relative focus:outline-none">
        ...
        <div className="flex-grow">
          <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-2 mb-1">
            ...
            {detail.date && <span className="text-[10px] font-black tracking-widest uppercase px-2 py-0.5 bg-slate-100 text-[#2596be] rounded-md self-start md:self-auto">{detail.date}</span>}
          </div>
          ...
          {detail.extendedText && (
            <div className="p-4 md:p-5 pt-0 bg-slate-50 border-t border-slate-100">
              <div className="bg-blue-50/70 p-4 rounded-lg text-sm text-slate-700 leading-relaxed flex gap-3 border border-blue-100 italic">
                ...
                <div><strong className="block mb-1 text-[#133c55] font-bold uppercase text-[11px] tracking-widest">Profundización:</strong>{detail.extendedText}</div>
              </div>
            </div>
          )}
        </div>
...
const WelcomeScreen = ({ onNext }) => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-center animate-fade-in px-4 py-8">
    ...
    <p className="text-lg md:text-xl text-slate-500 font-light mb-4">{infographicData.header.subtitle}</p>
    <p className="text-slate-600 max-w-xl mb-4">Hola, soy Valerio, tu coach de IA. En este laboratorio exploraremos la evolución de los modelos GPT, sus modos de operación, herramientas integradas y el arte del prompt estratégico.</p>
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <main className="max-w-3xl mx-auto px-4 md:px-6 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl md:text-5xl font-black mb-3 leading-tight tracking-tight text-[#133c55]">{infographicData.header.title}</h1>
          <p className="text-lg md:text-xl text-slate-500 font-light">{infographicData.header.subtitle}</p>
            ...
            return (
              <div key={section.id} className={`bg-white rounded-3xl shadow-sm border transition-all duration-500 overflow-hidden ${isActive ? 'border-[#2596be] ring-1 ring-[#2596be]/10' : 'border-slate-100 hover:border-slate-300'}`}>
                <button onClick={() => handleSectionClick(section.id)} className="w-full flex items-center justify-between p-6 md:p-8 text-left focus:outline-none bg-white transition-colors">
                  ...
                  <div className={`p-2 transition-all duration-500 ${isActive ? 'text-[#2596be] rotate-180' : 'text-slate-300'}`}><ChevronDown size={28} /></div>
                </button>
                ...
                <div className={`grid transition-all duration-500 ease-in-out ${isActive ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                  <div className="overflow-hidden">
                    <div className="p-6 md:p-8 pt-0 bg-slate-50/30">
                      <div className="flex flex-col md:flex-row gap-6 justify-between items-start mb-8 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                        <div className="flex-grow">
                          <p className="text-slate-700 leading-relaxed text-lg md:text-xl border-l-4 pl-6" style={{ borderColor: '#2596be' }}>{section.content}</p>
                        </div>
                        ...
                        <div className="mt-8 p-8 bg-[#133c55] text-white rounded-[2rem] text-center shadow-xl relative overflow-hidden group">
                          ...
                          <div className="flex flex-wrap justify-center gap-3 relative z-10">
                            {['Rol', 'Contexto', 'Tarea', 'Formato', 'Restricciones', 'Ejemplos'].map((elem, i) => (
                              <span key={i} className="px-5 py-2.5 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 text-sm font-bold transition-colors">{elem}</span>
                            ))}
                          </div>
                        </div>
                      )}
