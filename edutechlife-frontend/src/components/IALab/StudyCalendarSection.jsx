...
const buildActivityTypeMap = (year, month) => {
  ...
  const typeMap = {}
  const allEntries = [
    ...activities.map(a => ({ date: new Date(a.completed_at), type: a.activity_type })),
    ...sessions.map(s => ({ date: new Date(s.completed_at), type: 'session' })),
  ]
...
const buildCalendar = (year, month, months) => {
  ...
  return { year, month, days, todayDate, isPastMonth, label: `${months[month]} ${year}` }
}
...
const StudyCalendarSection = () => {
  ...
  const { completedExams, challengeScores, modules } = useIALabProgressContext() || {}
  ...
  const activityTypeMap = useMemo(() => buildActivityTypeMap(calYear, calMonth), [calYear, calMonth])
      ...
      modules.forEach(mod => {
        ...
        if (score === undefined || score === null || score < 80) {
          deadlines.push({
            ...
            label: t('ialab.study_calendar.pending_challenge', { modulo: mod.id }),
            moduleId: mod.id,
            type: 'challenge',
            icon: 'fa-trophy',
  ...
  const todayKey = formatDate(new Date().getFullYear(), new Date().getMonth(), new Date().getDate())
        ...
        <div className="bg-slate-50 dark:bg-slate-900/30 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1">
              <button onClick={goToPrevMonth} aria-label="Mes anterior" className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 flex items-center justify-center text-xs text-slate-600 dark:text-slate-300 transition-all">‹</button>
              <span className="text-xs font-bold text-petroleum dark:text-petroleum w-28 text-center">{calendar.label}</span>
              <button onClick={goToNextMonth} aria-label="Mes siguiente" className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 flex items-center justify-center text-xs text-slate-600 dark:text-slate-300 transition-all">›</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-7 gap-0.5 text-center">
            ...
            {calendar.days.map((d, i) => {
              ...
              const types = dateKey ? activityTypeMap[new Date(calendar.year, calendar.month, d).toDateString()] : null
              const isPastDay = calendar.isPastMonth || (d !== null && d < calendar.todayDate)
              return (
                <div
                  key={i}
                  ...
                  className={`relative text-xs py-2 rounded-lg cursor-pointer transition-all duration-150 flex flex-col items-center gap-0.5 ${
                    ...
                    isPastDay ? 'bg-petroleum/5 dark:bg-petroleum/10 text-slate-500 dark:text-slate-400 hover:bg-petroleum/10' :
          ...
          <div className="pt-2 mt-2 border-t border-slate-100 dark:border-slate-700">
            <div className="flex items-center gap-3 text-[10px] text-slate-400">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-purple-500" /> {ACTIVITY_LABELS.exam}</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500" /> {ACTIVITY_LABELS.challenge}</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-sky-500" /> {ACTIVITY_LABELS.video}</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-petroleum" /> {ACTIVITY_LABELS.session}</span>
            </div>
          </div>
        </div>
