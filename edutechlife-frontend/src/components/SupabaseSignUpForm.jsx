...
import { Brain, ArrowLeft, Eye, EyeOff, CheckCircle2, AlertCircle, Zap, Lock, Mail, User, Phone } from "lucide-react";
...
const SupabaseSignUpForm = ({ onBack, returnTo }) => {
  ...
  const passwordStrength = useMemo(() => {
    ...
    const levels = [
      ...
      { score: 1, label: t("signup.password_weak") || "Débil", color: "bg-red-500" },
      { score: 2, label: t("signup.password_fair") || "Regular", color: "bg-orange-500" },
      { score: 3, label: t("signup.password_good") || "Buena", color: "bg-yellow-500" },
      { score: 4, label: t("signup.password_strong") || "Fuerte", color: "bg-green-500" },
      { score: 5, label: t("signup.password_very_strong") || "Muy fuerte", color: "bg-emerald-500" },
    ];
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      if (!formData.firstName.trim()) newErrors.firstName = t("signup.error.first_name_required") || "Required";
      if (!formData.lastName.trim()) newErrors.lastName = t("signup.error.last_name_required") || "Required";
      if (!formData.email.trim()) newErrors.email = t("signup.error.email_required") || "Required";
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email))
        newErrors.email = t("signup.error.invalid_email") || "Invalid email";
    } else if (step === 2) {
      if (!formData.username.trim()) newErrors.username = t("signup.error.username_required") || "Required";
      if (!formData.password) newErrors.password = t("signup.error.password_required") || "Required";
      else if (formData.password.length < 8)
        newErrors.password = t("signup.error.password_min_length") || "At least 8 characters";
  ...
  const isStep1Valid = !formData.firstName.trim() || !formData.lastName.trim() || !formData.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email);
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#004B63] via-[#0A3550] to-[#1a5f7a] flex items-center justify-center p-4 relative overflow-hidden">
        ...
        >
          ...
          <span className="text-sm font-medium">{t("ialab.signup_back_to_login")}</span>
        </button>
                ...
                <div className="lg:w-2/5 bg-gradient-to-br from-[#004B63] to-[#4DA8C4] p-8 lg:p-12 text-white flex flex-col justify-between relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-12">
                      ...
                      <div>
                        ...
                        <p className="text-white/80 text-sm">{t("ialab.signup_subtitle")}</p>
                      </div>
                    </div>

                    <div className="mb-12">
                      <h2 className="text-4xl font-bold mb-6">{t("ialab.signup_welcome_title")}</h2>
                      <p className="text-white/90 leading-relaxed mb-4 text-lg" dangerouslySetInnerHTML={{
                        __html: sanitize(t("ialab.signup_welcome_desc")),
                      }} />
                      <p className="text-white/70 italic">{t("ialab.signup_welcome_quote")}</p>
                    </div>
                  ...
                  <div className="relative z-10 mt-8 pt-6 border-t border-white/20">
                    <p className="text-white/70 text-sm">
                      ...
                      <button onClick={onBack} className="text-white hover:underline font-semibold">
                  ...
                  <div className="mb-8">
                    <div className="flex justify-between mb-4">
                      <span className="text-sm font-medium text-[#004B63]">
                        {currentStep === 1 ? t("signup.step_1_info") || "Información personal" : t("signup.step_2_security") || "Seguridad"}
                      </span>
                      <span className="text-xs text-gray-500">Paso {currentStep}/2</span>
                    </div>
                    ...
                    <div className="space-y-6 animate-in fade-in">
                      <div className="text-center mb-8">
                        ...
                        <p className="text-gray-600">
                          Ingresa rápidamente a tu cuenta con las opciones disponibles
                        </p>
                      </div>
                  ...
                  {currentStep === 2 && (
                    <form onSubmit={handleSignUp} className="space-y-5 animate-in fade-in">
                          ...
                          <div className="mt-2">
                            <div className="flex items-center gap-2 mb-1">
                              <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div className={`h-full ${passwordStrength.color} transition-all`} style={{ width: `${(Math.min(formData.password.length, 20) / 20) * 100}%` }} />
                              </div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                              {t("signup.info.password_min_length") || "Mínimo 8 caracteres. Usa mayúsculas, números y símbolos."}
                            </p>
                          </div>
                        )}
                      ...
                      <div className="flex gap-3 pt-4">
                        ...
                        >
                          {loading ? "Creando cuenta..." : t("signup.button.register") || "Crear cuenta"}
                        </button>
                      </div>
                    </form>
                  )}
              ...
              <div className="min-h-screen lg:min-h-auto flex items-center justify-center p-8">
                <div className="text-center max-w-md">
                  ...
                  <p className="text-gray-600 mb-2">
                    {t("signup.success.registration_complete") || "Tu cuenta ha sido creada exitosamente."}
                  </p>
