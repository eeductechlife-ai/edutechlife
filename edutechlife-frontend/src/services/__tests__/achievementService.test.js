...
describe('achievementService', () => {
  describe('shouldUnlock', () => {
    ...
    it('unlocks social_butterfly when 3+ friends added', () => {
      const userData = {
        friendsList: [{ id: 'friend_1' }, { id: 'friend_2' }, { id: 'friend_3' }],
      };
  ...
  describe('buildUnlockPayload', () => {
    it('builds payload with new achievements', () => {
      const newUnlocks = [{ id: 'first_lesson', name: 'Primer Paso', points: 10 }];
    ...
    it('deduplicates achievements', () => {
      const existing = [{ id: 'first_lesson', unlockedAt: '2024-01-01T00:00:00Z' }];
    ...
    it('sets unlockedAt timestamp', () => {
      ...
      const payload = buildUnlockPayload(newUnlocks, []);
      const now = Date.now();
      const unlockedTime = new Date(payload.unlockedRewards[0].unlockedAt).getTime();
  ...
  describe('calculateAchievementPoints', () => {
    ...
    it('handles achievements with missing points', () => {
      const unlocked = [{ id: 'first_lesson' }, { id: 'hundred_points', points: 25 }];
