...
describe('predictionService', () => {
  describe('predictChurnRisk', () => {
    it('returns high risk when inactive > 7 days', () => {
      const studentData = {
        streak: { lastActive: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000), current: 0 },
        totalPoints: 30,
    ...
    it('penalizes broken streak', () => {
      const studentData = {
        streak: { current: 0, lastActive: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) },
        totalPoints: 100,
        missions: [{ id: '1' }],
      };
      const result = predictChurnRisk(studentData);
      expect(result.reasons.some((r) => r.toLowerCase().includes('racha') || r.toLowerCase().includes('streak'))).toBe(true);
    });
    ...
    it('returns low risk for active engaged student', () => {
      const studentData = {
        totalPoints: 300,
        streak: { current: 7, lastActive: new Date(Date.now() - 1 * 60 * 60 * 1000) },
    ...
    it('caps score at 100', () => {
      const studentData = {
        totalPoints: 0,
        streak: { current: 0, lastActive: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
        missions: [],
  ...
  describe('predictNextUnlock', () => {
    ...
    it('calculates days until next unlock', () => {
      const studentData = {
        totalPoints: 50,
        pointsHistory: [
          { date: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000), cumulative: 0 },
  ...
  describe('calculateDailyPoints', () => {
    ...
    it('rounds to nearest integer', () => {
      const studentData = {
        pointsHistory: [
          { date: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000), cumulative: 0 },
          { date: new Date(Date.now()), cumulative: 100 },
        ],
      };
      const dailyPoints = calculateDailyPoints(studentData);
  ...
  describe('Security and edge cases', () => {
    ...
    it('validates alert timestamp format', () => {
      ...
      const previous = [
        { type: 'test', createdAt: 'invalid-date' },
      ];
