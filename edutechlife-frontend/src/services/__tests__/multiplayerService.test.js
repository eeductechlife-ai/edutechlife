...
describe('multiplayerService', () => {
  const mockStudents = [
    { id: '1', name: 'Alice', totalPoints: 500, streak: { current: 10 }, analyzedActivities: [{ score: 95 }, { score: 90 }] },
    { id: '2', name: 'Bob', totalPoints: 400, streak: { current: 5 }, analyzedActivities: [{ score: 85 }] },
    { id: '3', name: 'Charlie', totalPoints: 300, streak: { current: 2 }, analyzedActivities: [{ score: 75 }] },
  ];
  ...
  describe('detectAnomalies', () => {
    ...
    it('returns no flags for normal progression', () => {
      const newData = { totalPoints: 110, totalActiveMinutes: 60, streak: { current: 3 } };
      const oldData = { totalPoints: 100, totalActiveMinutes: 55, streak: { current: 2 } };
      const result = detectAnomalies(newData, oldData);
  ...
  describe('calculateTrend', () => {
    it('calculates percentage growth over 7 days', () => {
      const studentData = {
        pointsHistory: [
          { date: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), cumulative: 100 },
          { date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000), cumulative: 200 },
        ],
      };
    ...
    it('handles edge case when starting points are 0', () => {
      const studentData = {
        pointsHistory: [
          { date: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), cumulative: 0 },
    ...
    it('returns negative trend for declining scores', () => {
      const studentData = {
        pointsHistory: [
          { date: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), cumulative: 200 },
          { date: new Date(Date.now()), cumulative: 100 },
        ],
      };
      const trend = calculateTrend(studentData, 7);
