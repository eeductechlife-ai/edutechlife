...
export const useIngenIAKids = () => {
  const context = useContext(IngenIAKidsContext);
  if (!context) {
    throw new Error("useIngenIAKids must be used within IngenIAKidsProvider");
  }
  return context;
};

// Safe version — returns null when used outside the provider (e.g. AppLayout)
export const useIngenIAKidsSafe = () => useContext(IngenIAKidsContext) ?? null;

export const IngenIAKidsProvider = ({ children }) => {
  ...
  const actions = useIngenIAActions({
    setTotalPoints,
    setPointsHistory,
    setMissions,
    missions,
  ...
  const value = {
    ...
    // Persisted data from other IngenIA tools
    flashcardDecks,
    setFlashcardDecks,
    examMaterials,
    setExamMaterials,
