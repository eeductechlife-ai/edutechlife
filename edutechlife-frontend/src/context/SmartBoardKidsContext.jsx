...
export const SmartBoardKidsProvider = ({ children }) => {
  ...
  const [missions, setMissions] = useState([
    ...
    {
      id: 6,
      title: "Lee 3 noticias tech",
      description: "Mantente al día con la tecnología",
      icon: "📰",
      xp: 80,
      completed: false,
    },
    ...
    const loadAllData = async () => {
      try {
        const localData = {
          ...
          // Backward-compat: try per-user key first, fall back to global key
          flashcardDecks: (() => {
  ...
  const setVakResultAndRecommendations = useCallback(
    (result) => {
      setVakResult(result);
  ...
  const addUploadedActivity = useCallback(
    (activity) => {
  ...
  const addAnalyzedActivity = useCallback(
    (analysis) => {
