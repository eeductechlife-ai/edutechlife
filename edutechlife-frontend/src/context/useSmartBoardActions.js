...
export const useSmartBoardActions = (stateAndSetters) => {
  ...
  const unlockReward = useCallback((reward) => {
    const { setUnlockedRewards, setDarkMode, setAvatarAnimado, setFondoGalaxia, setLastUnlockedReward } = ref.current
  ...
  const buildDaniContext = useCallback(() => {
    const { vakResult, totalPoints, streak, calendarEvents, studentMoodHistory, academicTopics, userId, conversationCount, missions, subjects, studentAge } = ref.current
    const now = new Date()
