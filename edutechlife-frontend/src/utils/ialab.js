 ...
 *
 * Keys are scoped to the signed-in account. Everything stored through here
 * (XP, streak, badges, completed modules/exams, lesson progress…) is per
 * student, and an unscoped key meant every account on the same browser shared
 * one record — three students would all see the first one's progress.
 */
export const ls = {
...
export const calcModuleScore = (mod) => {
  ...
  score +=
    mod.resourcesEarned || (mod.resourcesCompleted ? WEIGHTS.resources : 0);
...
export const analyzeQuizFailures = (storageGetFn) => {
  ...
  for (let id = 1; id <= 5; id++) {
    ...
    if (Array.isArray(attempts)) {
      attempts.forEach((a) => {
        if (Array.isArray(a.failedQuestions))
          latestFailures.push(...a.failedQuestions);
      });
    }
...
export const generateRecommendations = (
  ...
  for (let id = 1; id <= 5; id++) {
    ...
    const resourcesLeft = mod.viewedResources
      ? totalResources - mod.viewedResources.length
      : totalResources;
    ...
    if (resourcesLeft > 0) {
      recs.push({
        ...
        moduleName: modName,
        type: "resources",
        text: `Te faltan ${resourcesLeft} recursos por ver en ${modName}`,
        urgency: resourcesLeft > 4 ? "high" : "medium",
      });
    }
    if (examScore > 0 && examScore < 80) {
      recs.push({
        ...
        moduleName: modName,
        type: "exam",
        text: `Puedes mejorar tu examen de ${modName} (${examScore}%)`,
        urgency: "high",
      });
      ...
      recs.push({
        ...
        moduleName: modName,
        type: "exam",
        text: `Rinde el examen de ${modName}`,
        urgency: "high",
      });
    }
    if (challengeScoreVal > 0 && challengeScoreVal < 80) {
      recs.push({
        ...
        moduleName: modName,
        type: "challenge",
        text: `Mejora tu desafío de ${modName} (${challengeScoreVal}%)`,
        urgency: "medium",
      });
      ...
      recs.push({
        ...
        moduleName: modName,
        type: "challenge",
        text: `Completa el desafío de ${modName}`,
        urgency: "medium",
      });
    }
  }
