# Instrucciones para crear tabla student_grades en Supabase
...
### 2. Abrir SQL Editor
- En el menú izquierdo, hacer clic en "SQL Editor"
...
### 4. Ejecutar el SQL
- Hacer clic en el botón "Run" (▶️) en el SQL Editor
...
1. **`src/context/IALabContext.jsx`** - Agregado estado `showPremiumEvaluationModal`
2. **`src/components/IALab/IALabChallengeSection.jsx`** - Modificado botón "Iniciar Desafío Premium"
3. **`src/components/IALab/IALab.jsx`** - Agregado modal premium
4. **`src/components/IALab/IALabEvaluationModalPremium.jsx`** - Nuevo wrapper
5. **`src/hooks/IALab/useIALabEvaluation.js`** - Hook para evaluación con DeepSeek
6. **`src/components/IALab/IALabEvaluationModal.jsx`** - Modal refactorizado
7. **`src/components/IALab/IALabEvaluationStep[1-3].jsx`** - Componentes de los 3 pasos
8. **`src/components/IALab/IALabEvaluationResults.jsx`** - Dashboard de resultados
...
2. **Acceder al IA Lab:**
   - Navegar a la sección IA Lab
   - Hacer clic en "Iniciar Desafío Premium"

3. **Completar los 3 pasos:**
   - Paso 1: Identificar Rol/Contexto/Tarea (drag & drop)
   - Paso 2: Optimizar prompt mal redactado
   - Paso 3: Crear prompt desde cero

4. **Ver resultados:**
   - Nota global (0-100)
   - Feedback detallado de DeepSeek
   - Persistencia en Supabase

## 📊 Monitoreo

Para verificar que las notas se están guardando:

```sql
-- Ver todas las notas
SELECT * FROM student_grades ORDER BY created_at DESC;

-- Ver progreso por usuario
SELECT * FROM student_challenge_progress;

-- Ver mejores puntajes
SELECT * FROM get_best_challenge_score('user_id_here', 1);
```

## 🚨 Solución de problemas

### Error: "Tabla student_grades no encontrada"
- Verificar que la tabla se creó correctamente
- Ejecutar las consultas de verificación

### Error: "Permiso denegado para la tabla student_grades"
- Verificar que las políticas RLS están configuradas
- El usuario debe estar autenticado con Clerk

### Error: "DeepSeek API key no configurada"
- Verificar que `VITE_DEEPSEEK_API_KEY` está en `.env.local`
